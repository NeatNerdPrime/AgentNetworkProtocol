...
> Note: This release converges the v2 core into two paths: "self-service joining and direct addition":
>
> 1. `group.invite`, `group.accept_invite` and standard `invitation` objects have been moved out of the v2 core;
> 2. `membership_request`, `membership_request_digest`, `group.approve_membership`, and `group.reject_membership` have been moved out of the v2 core;
> 3. `group_policy` converges to `message_security_profile + bootstrap_security_profile + admission_mode + permissions`;
> 4. Non-member governance directed notification `group.governance_notice` has been moved out of v2 core;
...
6. Optional stable Handle membership anchors, current DID credential rebinding, and group identity continuity.
...
- **Member Handle**: A complete Handle conforming to WNS. When recorded in `group_member`, it is the permanent stable anchor for that membership.
- **Handle-backed Member**: A group member with a verified `member_handle`; the Handle identifies the stable membership and `agent_did` identifies its current cryptographic credential.
- **Handle Binding Generation**: The monotonic `binding_generation` from the WNS Handle Resolution Document, represented in P4 as the decimal string `handle_binding_generation`.
- **Member Credential Rebind**: Atomically replacing the current `agent_did` of a Handle-backed Member without changing its role, status, or join time.
...
### 3.8 Separation of Handle Membership Anchor and DID Credential

This Profile allows an Agent to become a member in either DID-only or Handle-backed mode:

- A DID-only membership is identified by its current `agent_did`; if the DID string changes, this Profile provides no automatic continuity.
- For a Handle-backed Member, `member_handle` is the stable membership anchor and `agent_did` is the current credential for authentication, signatures, routing, and security Overlays.
- A Handle-backed Member **MUST** use a permanently non-transferable and non-reusable WNS Handle. The Group Host **MUST** treat the Handle Provider as the continuity trust root for that Handle.
- If the Handle itself is `revoked`, a member credential rebind **MUST NOT** be performed. If the Handle remains `active` while its underlying DID changes, `group.rebind_member` is used.

On the ANP wire, the stable membership key for a Handle-backed Member is `(group_did, member_handle)`, while its `agent_did` may be rebound. The membership key for a DID-only member is `(group_did, agent_did)`. Implementations may map either form to a local User ID inside clients and servers, but that User ID does not enter ANP.

After a membership is established, DID-only and Handle-backed modes **MUST NOT** be silently converted or merged merely because they currently resolve to the same DID. An Agent that wants Handle continuity must submit the Handle when that membership is created; this version defines no method for attaching a Handle later to a DID-only membership.
...
| Rebind a Handle member's DID | `group.rebind_member` | The old DID loses authority and the new DID becomes current | `group_member` remains `active` | When the Group Host accepts the rebind | Handle-backed Members only |
...
When Handle-backed Members are used, this Profile also **MUST** depend on the [ANP DID-WBA Name Space Specification (WNS)](../../04-anp-did-wba-name-space-specification.md) for Handle format, resolution, bidirectional binding, `binding_generation`, permanent non-transferability, and permanent non-reuse semantics.
...
```mermaid
flowchart TD
COps[Group state-changing operations<br/>create / join / add / rebind / remove / leave / update_*]
Msg[group.send]
...
- v2 **MUST NOT** introduce custom roles within the minimum interoperability scope.
...
- `member_handle`: complete WNS Handle string, **MUST** for a Handle-backed Member and **MUST NOT** for a DID-only member
- `handle_binding_generation`: positive decimal string, **MUST** for a Handle-backed Member and **MUST NOT** for a DID-only member
- `role`: string, **MUST**
- `status`: string, **MUST**
...
- `member_handle` **MUST** be the complete Handle normalized under WNS and **MUST NOT** change during `group.rebind_member`;
- `handle_binding_generation` **MUST** exactly copy the WNS `binding_generation` verified when the membership was established or most recently rebound;
- A Group Host **MUST NOT** establish or rebind membership from Handle text alone; it must first complete WNS resolution and binding verification.

This Profile does not define or transmit `user_id`, `member_user_id`, or any other local database identifier. A client or server **MAY** project `group_member` onto a stable local key internally, but that key is not an ANP group-member identifier and **MUST NOT** be used for cross-domain authentication or authorization.
...
This Profile v2 recommends the following `event_type`:
...
- `member-credential-rebound` **MUST** contain `subject_handle`, `previous_subject_did`, `subject_did`, `handle_binding_generation`, and `membership_status = "active"`
...
12. `body.creator_handle` **MAY** exist

If `body.creator_handle` is present, the Group Host **MUST** resolve and verify it under WNS, the resolution status **MUST** be `active`, and its `did` **MUST** exactly equal `meta.sender_did`. On success, the creator's `group_member` **MUST** record `member_handle` and the corresponding `handle_binding_generation`; if the field is absent, the creator is a DID-only member.
...
- Each entry **MUST** contain exactly one of `member_did` or `member_handle`, and **MUST NOT** contain both or neither;
- A Handle entry **MUST** be resolved, verified, and stored according to the Handle path in Section 8.4;
...
If the request contains `creator_handle`, the successful response **MUST** contain `creator_handle` and `handle_binding_generation`; otherwise, both fields **MUST NOT** appear.
...
`group.join` is used for non-members to initiate joining independently. For v2 cores, the caller immediately becomes a member of `active` on success.
...
If `member_handle` is absent, the caller joins in DID-only mode and the member DID is `meta.sender_did`. If `member_handle` is present, the Group Host **MUST**:

1. Normalize and resolve the Handle and complete bidirectional binding verification under WNS;
2. Require the resolution document to have `status = "active"`;
3. Require the resolved `did` to exactly equal `meta.sender_did`;
4. Store the normalized `member_handle` and resolved `binding_generation` in the membership record.
...
- `group_did`
- `member_did`, which **MUST** equal `meta.sender_did`
...
If the request contains `member_handle`, the successful response **MUST** contain `member_handle` and `handle_binding_generation`; a DID-only join response **MUST NOT** contain either field.
...
`body` **MUST** contain exactly one target identifier:
...
If `member_did` is used, the Group Host **MUST** establish a DID-only member. If `member_handle` is used, the Group Host **MUST** normalize and resolve it and complete bidirectional binding verification under WNS, require the Handle to be `active`, and establish a Handle-backed Member using the resolved current DID. The membership record **MUST** store `member_handle`, the current `agent_did`, and `handle_binding_generation` together.
...
If the request uses `member_handle`, the successful response **MUST** contain `member_handle` and `handle_binding_generation`; if it uses `member_did`, the successful response **MUST NOT** contain either field.
...
`group.rebind_member` atomically replaces the current credential of a Handle-backed Member after its underlying DID changes. It does not create a new member, change the membership anchor, or recover a DID-only member.

A WNS mapping change only makes the new binding detectable and **MUST NOT** automatically rewrite group membership. Even if the Group Host observes a new DID through polling or cache refresh, it must wait for the new DID to submit and pass this method for each group. This method and its strictly increasing `handle_binding_generation` are therefore the standard mechanism by which a group learns of a DID change.

#### 8.12.2 Caller and Request Requirements

This method **MUST** be initiated by the new DID and satisfy:

- `meta.security_profile` **MUST** equal `transport-protected`;
- `meta.sender_did` **MUST** equal `body.new_member_did`;
- `auth.origin_proof` **MUST** be produced by a valid `authentication` verification method of the new DID;
- `body` **MUST** contain `member_handle`, `previous_member_did`, `new_member_did`, and `handle_binding_generation`.

The recommended `body` is:
...
Before accepting the request, the Group Host **MUST**:

1. Locate exactly one membership in the same group with `status = "active"`, the exact `member_handle`, and current `agent_did = previous_member_did`;
2. Perform fresh WNS resolution and bidirectional binding verification for `member_handle`, without relying only on a cached result beyond its current TTL;
3. Require the resolution document to have `status = "active"` and its `did` to exactly equal `new_member_did`;
4. Require its `binding_generation` to exactly equal `handle_binding_generation` and, when compared as a decimal integer, be strictly greater than the stored old `handle_binding_generation`;
5. Verify the new DID Document and proof of possession of the new DID; a signature by the old DID alone is not sufficient continuity proof;
6. Confirm that `new_member_did` does not identify another `active` membership in the same group;
7. Fail closed if the Handle is `suspended`, `revoked`, cannot be verified under WNS, or any concurrent conflict occurs.

After successful verification, the Group Host **MUST**, in one atomic state change:

- Update the membership's `agent_did` to `new_member_did`;
- Update `handle_binding_generation` to the new value;
- Preserve `member_handle`, `role`, `status`, `joined_at`, `added_by`, and the active member count;
- Advance `group_state_version` and `group_event_seq`;
- Immediately stop authorizing `previous_member_did` to act for that membership;
- Emit a `member-credential-rebound` state event.

Historical messages, `group_receipt` objects, events, DIDs, and signatures **MUST NOT** be rewritten.

#### 8.12.3 Successful Response
...
- `member_handle`
- `previous_member_did`
- `member_did`, whose value is the new DID
- `handle_binding_generation`
- `membership_status`, which **MUST** be `active`
- `group_state_version`

A successful response **MAY** contain `group_receipt`.
...
### 9.4 DID rebind for a Handle-backed Member
...
sequenceDiagram
    ...
    participant M as Other active members
...
`group.rebind_member` is a recovery control operation, not an ordinary member-only operation that an existing MLS member can produce. It **MUST** use `transport-protected` and the new DID's `origin_proof` for the P4 rebind. If the group uses E2EE, the current owner as P6 MLS Controller **MUST** automatically orchestrate `group.e2ee.add(new DID)` followed by `group.e2ee.remove(old DID)`, while the Group Host **MUST** pause application messages as required by P6. The P4 proof **MUST NOT** substitute for either MLS Commit.
...
### 11.9 Handle Resolution and Rebind Security

- A Handle is only a stable membership anchor after WNS verification and **MUST NOT** replace DID request signatures;
- A Handle Provider **MUST** commit to permanent non-transferability and non-reuse and retain revoked Handles as permanent tombstones. A Handle that does not satisfy this commitment **MUST NOT** be used for Handle-backed membership;
- The Group Host **MUST** persist the last accepted `handle_binding_generation` and reject an equal, older, or non-decimal-comparable generation;
- If a cached Handle resolution result has expired or conflicts with the requested newer generation, the Group Host **MUST** resolve it again and fail closed;
- `group.rebind_member` is a narrowly scoped exception that the new DID may invoke before becoming the current `agent_did`; its authority is limited to the same-Handle credential rebind defined in Section 8.12;
- How deployments invalidate old-DID tokens, sessions, and routing caches is implementation-specific, but after an accepted rebind the old DID **MUST NOT** pass P4 authorization for that membership.
...
| 3011 | `group.invalid_handle_binding` | The Handle format, status, resolution result, or bidirectional binding is invalid |
| 3012 | `group.handle_binding_stale` | The Handle binding generation did not strictly increase, rolled back, or came from a stale cache |
| 3013 | `group.rebind_not_allowed` | The target is not an eligible Handle-backed active member, or the Handle is suspended or revoked |
| 3014 | `group.rebind_conflict` | The new DID belongs to another active member, or a concurrent rebind conflict occurred |
...
Choosing Handle-backed membership discloses the complete `member_handle` to callers authorized to read that roster field. Implementations **SHOULD** make this privacy boundary clear; public or anonymous roster views still **SHOULD NOT** expose Handles by default.
...
13. `group_event_seq`
14. DID-only and Handle-backed membership modes
15. Role: `owner`, `admin`, `member`
16. Member status: `active`, `left`, `removed`
17. Unified event object semantics of `group.state_changed`, including `member-credential-rebound`
...
### 15.5 `group.rebind_member` Example
...
- Implementations may use a local `user_id` for internal data association, but P4 wire objects use only a current DID and an optional stable Handle;
- A Handle-backed Member should preserve membership continuity through `member_handle` and treat `agent_did` as its replaceable current credential;
- A DID-only member has no automatic continuity when its DID string changes and implementations must not guess or merge identities;
