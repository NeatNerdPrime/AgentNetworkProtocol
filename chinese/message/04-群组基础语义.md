...
- 当 `membership_mode = "invitation-only"` 时，不带有效邀请上下文的 `group.join` **SHOULD** 被拒绝；
...
被邀请 Agent 接受邀请，并进入后续治理流程。接受邀请 **不必然** 立即使其成为 `active` 成员；若群策略要求管理员审批或后续密码学入群收尾，则 Group Host **MUST** 将其置为 `pending`。
...
在群策略允许时，由某 Agent 主动申请或完成自助加入该群。若群策略要求管理员审批，Group Host **MUST** 先记录待审批治理请求，而 **MUST NOT** 在审批完成前把该 Agent 视为 `active` 成员。
...
由具备审批权限的成员或治理服务批准一个待处理的 `membership_request`。批准本身表示治理条件已满足；是否立即使目标成为 `active` 成员，由当前群策略以及叠加的安全 Profile 共同决定。
