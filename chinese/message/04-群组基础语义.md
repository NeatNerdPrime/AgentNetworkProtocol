...
- `custom`：对象，**MAY**
- `required_security_profile`：字符串，**MAY**，**仅为向后兼容的 deprecated alias**；若仅出现该字段，接收方 **MUST** 将其按 `message_security_profile` 解释

语义约定如下：

- `message_security_profile` 约束 `group.send` 以及已激活成员的 member-only 群操作；
- `bootstrap_security_profile` 约束 `group.join`、`group.accept_invite`、`group.approve_membership` 等 onboarding / bootstrap 路径；
- 当 `bootstrap_security_profile` 省略时，接收方 **SHOULD** 将其解释为 `"transport-protected"`；
- 当 `message_security_profile` 省略但 `required_security_profile` 存在时，接收方 **MUST** 将 `required_security_profile` 解释为 `message_security_profile`。
...
当 did:wba 认证信息承载在 ANP JSON-RPC 群请求中时，为保证 `actor_proof` 能跨服务转发保持稳定，本 Profile 对 `Signature-Input` 中的关键组件作如下应用层映射：

- `@method`：映射为当前群业务方法名，例如 `group.create`、`group.send`、`group.add`
- `@target-uri`：
  - 对 `group.create`：`anp://service/<pct-encoded serviceDid>`
  - 对面向既有群的请求：`anp://group/<pct-encoded group_did>`
- `content-digest`：映射为 Signed Group Payload 的摘要值

验证方 **MUST** 使用上述映射重建 signature base，而 **MUST NOT** 直接使用某一跳 HTTP URL 代替逻辑目标 URI。
### 7.10 `group_receipt`
...
- 对 `group.join`、`group.accept_invite`、`group.approve_membership` 等 onboarding / bootstrap 方法，服务端 **MAY** 允许 `transport-protected` 请求，但其允许与否 **SHOULD** 由 `bootstrap_security_profile` 明确约束。

若 `bootstrap_security_profile = "group-e2ee"`，则相关 onboarding / bootstrap 方法在进入生效路径前也 **MUST** 满足 Group E2EE Profile 的约束。
