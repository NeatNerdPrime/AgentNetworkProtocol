...
> 说明：本修订稿将 v1 核心收敛为“邀请加入、开放加入、直接加人”三条路径：
>
> 1. `membership_request`、`membership_request_digest`、`group.approve_membership`、`group.reject_membership` 已移出 v1 核心；
> 2. `group_policy` 收敛为 `message_security_profile + bootstrap_security_profile + admission_mode + permissions`；
> 3. 非成员治理定向通知 `group.governance_notice` 已移出 v1 核心；
> 4. 保留 `group.state_changed` 作为群内有序状态通知；
> 5. `group.send` 不再携带 `expected_group_state_version`。
...
2. 群的创建、邀请、接受邀请、开放加入、直接加人、移除成员、离群、更新群资料、更新群策略等基础动作；
...
- **Admission Mode**：群对非成员默认开放的入群路径。本 Profile v1 的标准值为 `invite-only`、`admin-add`、`open-join`。其中，中文“自动加入”在线协议取值上统一写作 `open-join`。
- **Policy**：决定谁可以发消息、邀请、直接加人、移除成员、更新资料和更新策略的应用层规则。
...
v1 核心中，`group.invite` 只建立邀请，不直接使目标成为 `active` 成员；`group.accept_invite` 表示被邀请方接受邀请，并在邀请仍然有效且策略允许时使成员资格立即生效。
...
| 接受邀请 | `group.accept_invite` | 接受有效邀请 | `invitation.status = accepted` | 在本次接受中立即生效 | 邀请路径第二步 |
| 开放加入 | `group.join` | 自助加入 | `group_member.status = active` | 在本次加入中立即生效 | 仅适用于 `open-join` |
...
| 管理员移除成员 | `group.remove` | 邀请被撤销或成员被移出群 | `invitation.status = revoked` 或 `group_member.status = removed` | 不适用 | 适用于 `invited`、`active` |
...
| `invitation` | `accepted` | 邀请已被接受，对应成员资格已生效 |
| `invitation` | `revoked` / `expired` | 邀请已终止，不再可用 |
| `group_member` | `invited` | 目标已存在邀请中的成员关系，但尚未成为 `active` |
...
stateDiagram-v2
    ...
    NonMember --> Invited: group.invite
    Invited --> Active: group.accept_invite
    Invited --> Removed: group.remove / revoke / expire
...
6. 默认解释规则如下：
   - 当 `admission_mode = "invite-only"` 时，`group.join` **MUST** 被拒绝；标准入群路径是 `group.invite` / `group.accept_invite`
   - 当 `admission_mode = "admin-add"` 时，`group.join` **MUST** 被拒绝；典型入群路径是 `group.add`
   - 当 `admission_mode = "open-join"` 时，`group.join` **MUST** 被允许
   - `group.invite` 与 `group.add` 是否可用，仍由 `permissions.invite` 与 `permissions.add` 决定
...
- `status = invited` 表示当前存在尚未生效的邀请关系；
...
本节的 proof 承载规则遵循 P1 附录 A 的共享 Origin Proof 约定；P4 仅定义 `actor_proof` 的业务语义、Signed Group Payload 与签名组件映射。
...
- `@target-uri`：
  - 当方法为 `group.create` 时，映射为 `anp://service/<pct-encoded meta.target.did>`
  - 当方法面向既有群时，映射为 `anp://group/<pct-encoded target group_did>`
...
```json
{
  ...
  "group_event_seq": "129",
  "subject_method": "group.accept_invite",
...
2. 它 **MUST NOT** 用于向尚未成为群成员的对象发送邀请、提醒或结果通知；
3. 它 **SHOULD** 与 `group_event_seq` 保持一致的顺序语义；
4. 若 `subject_method = "group.accept_invite"`、`"group.join"` 或 `"group.add"`，则 `event_type = "member-activated"`。
...
- `invitation-created` **MAY** 包含 `invitation_id`、`subject_did`
- `invitation-accepted` **MAY** 包含 `invitation_id`、`subject_did`
- `invitation-revoked` **MAY** 包含 `invitation_id`、`subject_did`
...
若返回 `member_list`，其内容 **SHOULD** 仅包含当前非终止关系，即 `invited` 与 `active`。

### 8.3 `group.invite`

#### 8.3.1 语义

由有权限的成员向某 Agent 发出加入邀请。`group.invite` **仅** 建立邀请对象与 `invited` 成员关系；它 **MUST NOT** 自动使 `invitee_did` 成为 `active` 成员。
...
本 Profile **不**标准化邀请对象向被邀请方的定向投递机制。调用方若需让被邀请方获知 `invitation_id`，**SHOULD** 通过带外渠道或通过 `direct.send` 传递。

### 8.4 `group.accept_invite`

#### 8.4.1 语义

被邀请 Agent 接受一个仍然有效的邀请；若邀请有效且策略允许，成员资格立即生效为 `active`。
...
`group.join` 仅用于 `admission_mode = "open-join"` 的群。成功时调用方立即成为 `active` 成员。
...
- 若目标当前处于 `invited`，则 `group.remove` **MUST** 使对应 `invitation.status = "revoked"`，并使对应成员关系终止；
...
- 若存在 `params.auth`，则：
  - `params.auth.scheme` **MUST** 等于 `anp-rfc9421-origin-proof-v1`
  - `params.auth.actor_proof` **MUST** 为原始 `actor_proof` 的无损副本
  - 中间服务 **MUST NOT** 重写新的业务 proof。

### 8.13 `group.state_changed`

`group.state_changed` 是群状态变化的标准异步通知方法，用于向当前活跃成员同步已排序的成员状态变化、邀请状态变化、群资料变化和群策略变化。它 **MUST** 作为 Notification 使用。
...
`group.state_changed` **MUST NOT** 用于邀请投递、非成员提醒或任何替代 `direct.send` 的定向治理通知。

---

## 9. 流程图总览（非规范性）

### 9.1 邀请加入路径（`invite-only`）
...
sequenceDiagram
    ...
    A->>H: group.invite
    H-->>A: invitation_id + group_state_version
    A-->>B: 带外或 direct.send 传递 invitation_id
    B->>H: group.accept_invite
    H-->>B: membership_status = active
    H-->>A: group.state_changed(member-activated)
```

### 9.2 开放加入路径（`open-join`）
...
3. 只有未命中幂等记录时，才继续检查 `expected_group_state_version` 等前置条件。

若接收方收到的期望版本与当前版本不一致，**MUST** 拒绝，或 **MUST** 返回明确的冲突语义。

### 10.3 群消息与状态版本

`group.send` **MUST NOT** 携带 `expected_group_state_version` 作为 v1 核心字段。
...
- 对 `group.join`、`group.accept_invite` 以及后续 Overlay 明确定义的 onboarding / bootstrap 方法，发送方 **MUST** 使用 Group E2EE Profile；
...
`invitation_id` 是可操作标识。实现方 **SHOULD** 避免把它暴露给无关方，并 **SHOULD** 优先通过受控渠道、带外渠道或受保护私聊传递。
...
16. 成员状态：`invited`、`active`、`left`、`removed`
17. `invitation` 对象
18. `group.state_changed` 的统一事件对象语义
19. 安全传输运行方式
...
```json
{
  ...
  "params": {
    ...
    "body": {
      "invitee_did": "did:wba:b.example:agents:bob:e1_<fingerprint>",
...
```json
{
  ...
  "params": {
    ...
    "body": {
      "reason_text": "订阅公开群",
      "expected_group_state_version": "7"
    }
  }
}
```
