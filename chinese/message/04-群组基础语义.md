...
> 说明：本版在重排章节布局的基础上，继续完善群治理定向通知：统一由 `Group Host Service` 通过新的 `group.governance_notice` 方法发出；`membership_request` 被明确为 Group Host 保存的权威治理对象，而 `group_governance_notice` 仅表示该权威对象的定向投递快照；并新增统一的 `reason_text` 字符串，用于在邀请、申请加入、接受邀请和加人提案等场景中表达便于审核的人类可读原因。
...
`group_governance_notice` 表示由 `Group Host Service` 发往特定参与方的群治理定向通知对象。它不是新的治理请求本身，而是某个权威治理对象（例如 `membership_request` 或 `invitation`）在特定时刻的**定向投递快照**。
...
- 申请人待知晓的审批结果；
- 被邀请方收到的邀请；
- 被邀请方、申请人或被直接加入方待继续完成后续入群收尾；
- 被直接加入方收到的结果通知；
- 被移除成员收到的结果通知。

`group_governance_notice` 的标准互通方法为 **`group.governance_notice`**。该通知 **MUST** 由 `Group Host Service` 逻辑上发出；在跨域场景中，它 **MAY** 直接发送给群外的目标 Agent。外层传输可以经过联邦服务转发，但从协议语义上，该通知始终是 Group Host 对特定参与方发出的群治理通知。
...
- `membership_request_id`：字符串，**MAY**。若该通知对应某个权威治理请求，则此字段引用该对象。对面向审批人的待审批通知，此字段 **MUST** 存在。
- `membership_request`：对象，**MAY**。表示由 Group Host 当前权威 `membership_request` 派生的只读快照，用于帮助接收方展示和预判，不是后续审批的权威输入。对面向审批人的待审批通知，此字段 **MUST** 存在。
...
1. `group_governance_notice` 是定向投递快照，不是新的群治理方法本身；它 **MUST NOT** 被当成 `group.approve_membership` / `group.reject_membership` 的权威输入。
2. `membership_request` 是 `Group Host Service` 保存的权威治理对象；通知中的 `membership_request` 字段只是只读快照。若二者不一致，**MUST** 以 Group Host 当前保存的权威对象为准。
3. `notice_type = membership-request-pending` 用于表达“某个 `membership_request` 仍待处理”。它**统一覆盖** `join-request`、`add-proposal` 和 `invite-acceptance` 三类待审批场景；接收方 **MUST** 通过 `membership_request.request_kind` 区分具体语义。
4. 对 `notice_type = membership-request-pending`，`membership_request_id` 和 `membership_request` 快照 **MUST** 同时存在。该快照 **MUST** 至少包含：`membership_request_id`、`request_kind`、`requester_did`、`target_did`、`status`、`pending_reason`；当存在时，还 **SHOULD** 包含 `invitation_id`、`proposed_role`、`reason_text`、`changed_at`、`membership_request_digest`、`expires_at`。
5. `notice_type = invitation-received` 表示被邀请方收到邀请。它 **不是** `group.invite` 方法本身的替代，而是 `group.invite` 被 Group Host 接受后向被邀请方发出的通知；该类型 **MUST** 携带 `invitation_id`，且 **MAY** 不带 `membership_request_id`。
6. `notice_type = membership-approved` 与 `membership-rejected` 用于表达某个成员资格治理请求已被批准或拒绝；若对应某个 `membership_request`，则该类型 **MUST** 携带 `membership_request_id`，并 **SHOULD** 携带最新的只读 `membership_request` 快照。若后续仍需密码学入群收尾，则 `required_action` **SHOULD** 取 `complete-join`。
7. `notice_type = member-added` 与 `member-removed` 用于表达目标已被直接加入群或已被移出群；这两类通知 **MUST** 携带 `subject_did`；若其来源于先前已存在的治理请求，则 `membership_request_id` **MAY** 存在。
8. 对面向审批人的待审批通知，接收方后续调用 `group.approve_membership` / `group.reject_membership` 时，**MUST** 使用通知中给出的 `membership_request_id` 作为权威对象标识。
9. 审批接口仍以 `membership_request_id` 为核心输入；通知中的 `membership_request` 只读快照仅用于帮助接收方在不额外查询的情况下展示和判断上下文。
10. 为降低基于过期通知审批的风险，通知中的 `membership_request` 快照 **SHOULD** 携带 `changed_at` 和/或 `membership_request_digest`；审批调用方 **MAY** 将其分别通过 `expected_changed_at` 和 `expected_membership_request_digest` 回传给 Group Host。
11. 若部署采用 notification-driven 治理流程，v1 **不要求** 标准互通的 `list/get membership_request` 方法；审批人可以直接依据通知中的 `membership_request_id` 与只读快照调用 `group.approve_membership` / `group.reject_membership`。
12. 为证明该通知确由 Group Host / Group DID 所认可，跨域或可迁移场景中的 `group_governance_notice` **MUST** 至少提供以下之一：
...
13. 若该通知对应某个已被 Group Host 接受并排序的治理动作，则 `group_receipt` **SHOULD** 存在；若该通知仅用于表达一个待处理动作提醒且尚无可迁移 `group_receipt`，则 `proof` **SHOULD** 存在。

#### 7.11.1 按 `notice_type` 的最小字段要求

| `notice_type` | 最小必需字段 | 说明 |
|---|---|---|
| `membership-request-pending` | `group_did`、`recipient_did`、`membership_request_id`、`membership_request`、`group_state_version`、`issued_at` | 面向审批人的统一待审批通知；具体是 `join-request`、`add-proposal` 还是 `invite-acceptance`，由 `membership_request.request_kind` 区分 |
...
- `reason`：字符串，**MAY**。表示审批人对本次批准给出的补充说明。
...
- `reason`：字符串，**MAY**。表示审批人对本次拒绝给出的补充说明。
...
以下三个 Notification / 异步消息方法中，`group.incoming` 与 `group.state_changed` 为可选传输便利能力；`group.governance_notice` 用于群治理定向通知，其标准语义由本节定义。
...
`group.governance_notice` **MUST** 逻辑上由 `Group Host Service` 发出。跨域场景中，它 **MAY** 直接发送给群外目标 Agent 所在域，但无论经过何种联邦转发，通知中的群授权语义 **MUST** 以 `group_governance_notice.group_receipt` 或 `group_governance_notice.proof` 为准。
...
- `meta.sender_did` **SHOULD** 等于 `body.notice.group_did`，用于表达该通知在逻辑上由对应群发出
- `body.notice` **MUST** 存在，且类型为 `group_governance_notice`
...
- `body.notice`：对象，**MUST**。表示当前投递的群治理通知对象，类型必须为 `group_governance_notice`。
- `body.notice.recipient_did`：字符串，**MUST**。表示该通知的目标接收方；投递层 **SHOULD** 把通知送达给与该 DID 对应的 Agent 服务。
- `body.notice.membership_request_id`：当通知对应某个治理请求时 **MUST** 存在。审批人随后调用 `group.approve_membership` / `group.reject_membership` 时 **MUST** 使用它作为权威对象标识。
- `body.notice.membership_request`：当通知对应某个治理请求且需要接收方直接理解上下文时 **SHOULD** 存在；对 `membership-request-pending` 则 **MUST** 存在。它只是只读快照，不是审批的权威输入。

对 `group.governance_notice`，实现 **MUST NOT** 仅凭外层服务发送者身份推定该通知已被群授权；接收方 **MUST** 验证 `body.notice.group_receipt` 或 `body.notice.proof`。
...
21. `group.governance_notice` 方法以及 `group_governance_notice` 对象的标准语义
22. 安全传输运行方式
