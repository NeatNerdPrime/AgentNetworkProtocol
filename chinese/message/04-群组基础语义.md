...
>
> 1. `group_policy` 调整为 `permissions + join_policy` 两层对象；
...
由具备审批权限的成员或治理服务批准一个待处理的 `membership_request`。它表示审批环节已完成；是否已经满足全部入群前置条件，仍取决于当前群策略以及是否还需要后续收尾。
...
- 对 `group.join`、`group.accept_invite`、`group.approve_membership` 等 onboarding / bootstrap 方法，服务端 **MAY** 允许 `transport-protected` 请求，具体由对应 Overlay 约束。
