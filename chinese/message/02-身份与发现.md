...
Agent DID 文档 **SHOULD** 至少包含一个 `ANPMessageService`。
...
- 一个 Agent DID 文档 **SHOULD** 至少包含一个 `ANPMessageService`
- 一个 Group DID 文档 **MUST** 至少包含一个 `ANPMessageService`
- 若该 `ANPMessageService` 会参与跨域服务到服务调用，则其服务条目 **MUST** 声明 `serviceDid`
- 若存在多个 `ANPMessageService`，调用方 **SHOULD** 根据 `profiles`、`securityProfiles`、`priority` 或本地策略选择
