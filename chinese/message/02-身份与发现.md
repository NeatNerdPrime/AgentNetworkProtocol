...
4. 结合 `profiles`、`securityProfiles`、能力协商结果与本地策略，确定该统一入口上可用的具体能力；
5. 如需显式协商能力，调用同一入口上的 `anp.get_capabilities`。
...
3. 结合 `profiles`、`securityProfiles`、能力协商结果与本地策略，确定该统一入口上可用的群管理、群消息、加入或群 E2EE 相关能力；
4. 若启用群 E2EE，则通过同一入口读取与群加密 Profile 绑定所需的引导信息。
