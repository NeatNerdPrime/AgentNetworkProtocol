...
- 发现直接消息 E2EE 所需公开材料
- 发现群 E2EE 所需的公开材料与 onboarding / bootstrap 相关材料；成员侧加入材料按对应 Agent DID 的统一入口发现，`welcome` / `ratchet_tree` 等密码学结果由后续 notice 交付
...
Join Role 表示加入与 onboarding / bootstrap 相关的入口能力；带外邀请凭据若存在，属于部署扩展，而不是 v1 核心能力。
...
7. 对 service-scoped 方法，调用方 **SHOULD** 把 `meta.target.did` 绑定到目标公开 `ANPMessageService.serviceDid`；对象控制面、密钥材料方法与群 E2EE onboarding / bootstrap 相关方法均遵循这一原则，除非具体 Profile 明确声明为 endpoint-local。
...
群成员关系、角色授权、部署扩展的入群凭据状态、成员状态、策略版本、群状态摘要等对象，**SHOULD** 由：
