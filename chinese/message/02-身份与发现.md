...
- `serviceDid` 参与的是跨域服务到服务身份认证，**不**直接参与 P4 的 `owner/admin/member` 群业务角色判定。
...
其协议级公开入口 **MUST** 仍通过该 DID 暴露的统一 `ANPMessageService` 进入。对象字节数据面可以是独立的 HTTP(S) 下载或上传端点，但对象控制面的对外发现锚点仍是该统一入口。
...
- 作为对象 HTTP(S) 下载入口发现与附件控制面发现的服务锚点（若部署支持）
...
- 对任何 service-scoped 方法，调用方 **SHOULD** 以该 `ANPMessageService.serviceDid` 作为目标服务身份锚点，除非对应 Profile 明确另有规定。
...
当业务对象（例如附件清单中的 `control_service_did`、或某个控制面返回对象中的等价字段）显式提供服务控制锚点时，调用方 **SHOULD** 优先按该 `serviceDid` 进行发现，而不是根据 `object_uri` 或其它 URL 反推控制面服务身份。
...
同理，`serviceDid` 只表示跨域服务身份，不应被机械解释为群内 `owner/admin/member` 角色。
