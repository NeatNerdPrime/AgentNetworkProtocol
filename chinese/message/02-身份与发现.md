...
- **Home Service**：`ANPMessageService` 背后的 Agent 主入口能力，用于接收直接消息、返回能力信息、指示进一步的交互路径。
- **Key Service**：`ANPMessageService` 背后的公开密钥材料或引导材料能力。
- **Group Service**：`ANPMessageService` 背后的群主入口或 Host 能力，用于群管理、群消息路由或群状态访问。
- **Join Service**：`ANPMessageService` 背后的加入、邀请、审批能力。
- **Capability Service**：`ANPMessageService` 背后的能力协商能力。
- **Object Service**：`ANPMessageService` 背后的对象上传、提交、下载票据签发和对象下载入口发现能力。
- **联邦服务 DID**：部署方在跨域服务到服务 HTTP 身份认证中使用的服务 DID，通常由 `ANPMessageService.serviceDid` 字段声明。对于 `did:web`，通常为 `did:web:{domain}`；对于 `did:wba`，通常为 `did:wba:{domain}` 这类裸域名 DID。
...
尤其对于 `group_receipt`、群治理通知中的 `proof` 以及其它“群已接受某结果”的可迁移见证对象，验证方 **MUST** 使用 `assertionMethod`。
...
- 若存在多个非跨域可调用的 `ANPMessageService`，调用方 **SHOULD** 根据 `profiles`、`securityProfiles`、`supportedMethods`、`logicalRoles`、`priority` 或本地策略选择

### 7.3 `ANPMessageService` 的逻辑角色

为保留跨实现语义清晰性，本 Profile 继续使用下列“逻辑角色”描述统一服务入口背后的能力边界；这些角色 **不是** DID 文档中的独立标准 service type。
...
- 对于 `did:web`，通常使用 `did:web:{domain}`；
- 对于 `did:wba`，通常使用 `did:wba:{domain}` 这类裸域名 DID。
...
- 要求：**SHOULD**
- 说明：
  - `authSchemes` 仅描述 hop-level 或 caller-level 认证能力；
...
3. 对于 `did:web` 与 `did:wba` 部署，`serviceDid` **SHOULD** 优先使用裸域名 DID，例如 `did:web:example.com` 或 `did:wba:example.com`；
4. `serviceDid` 表达的是“哪个服务身份在执行跨域调用”，而不是 `meta.sender_did` 所表达的业务主体身份；
5. `serviceDid` 的具体校验流程由 P8 定义；
6. 只有在满足 P8 所定义的 endpoint-origin / service binding 校验后，`serviceDid` 才能作为服务身份信任锚；否则它仅是发现提示信息。
...
2. 读取 `service` 列表；
3. 选择适当的 `ANPMessageService`；
4. 若后续涉及跨域服务到服务调用，读取该服务条目的 `serviceDid`；
5. 结合 `profiles`、`securityProfiles`、能力协商结果与本地策略，确定该统一入口上可用的具体能力；
...
2. 读取 `ANPMessageService`；
3. 若后续涉及跨域服务到服务调用，读取该服务条目的 `serviceDid`；
4. 结合 `profiles`、`securityProfiles`、能力协商结果与本地策略，确定该统一入口上可用的群管理、群消息、加入或群 E2EE 相关能力；
5. 若启用群 E2EE，则通过同一入口读取与群加密 Profile 绑定所需的引导信息。

### 9.3 Service DID 发现

当调用方已知某个 service-scoped 方法的目标 `serviceDid`（例如 `group.create` 的目标群 Host service）时，发现流程如下：
...
5. 仅在通过 P8 规定的服务身份绑定校验后，方可将该 `serviceDid` 视为可信跨域服务身份。
