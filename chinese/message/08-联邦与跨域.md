...
- 部署方 **SHOULD** 使用 `direct.send` 或语义等价的定向通知机制，把这些治理通知直接发送到对应 Agent 所在域。
...
当 did A 的服务向 did B 的服务发送跨域请求时，B 侧服务 **MUST** 按如下顺序校验发送方服务身份：

1. 从业务请求中获得原始业务主体 DID（例如 `meta.sender_did`）；
2. 解析该 DID 的 DID 文档；
3. 根据 P2 的服务选择规则，选择发送方对应的 `ANPMessageService`；
...
6. 把 `attachment.get_download_ticket` 送到最终 Object Service；
7. 明确禁止对象字节通过跨域服务调用链路常规转发；
8. 至少支持一种已排序群事件向成员域分发的机制，并支持对非成员参与方使用定向通知机制；
9. 为参与跨域调用的 `ANPMessageService` 声明 `serviceDid`，并在外层 HTTP Message Signatures 的 `keyid` 中使用该 DID 完成服务到服务身份认证；
10. 对 `did:wba` 部署，支持使用裸域名 DID 作为域级联邦服务 DID。
