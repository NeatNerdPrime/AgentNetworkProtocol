...
- 群操作跨域直接使用 `group.create`、`group.send`、`group.join`、`group.add`、`group.approve_membership`、`group.reject_membership`、`group.remove` 等原有群方法；
...
对于 `group.join`、`group.add`、`group.approve_membership` 这类方法，跨域成功 **不必然** 等同于目标已经成为 `active` 成员或已经进入 MLS 成员集；若 Group Host 返回的是 `pending` 或其它治理中间态，则发送方域服务 **MUST** 以 Group Host 的治理结果为准。
...
对于仍不是群成员的参与方，例如加入申请人、待审批的被加人目标或审批人：

- 与其相关的加入通知、审批通知、审批结果通知 **SHOULD NOT** 通过群成员专用的 `group.incoming` 分发；
...
对于 `group.join`、`group.add`、`group.approve_membership`、`group.reject_membership`、`group.remove`、`group.update_profile`、`group.update_policy` 等操作：

- 当最终 Group Host Service 接受并排序后，发送方域服务 **MAY** 向本地调用方返回成功；
- 若 Group Host 返回的是加入申请、加人请求、审批、待决或拒绝等治理结果，则该成功仅表示治理状态已被记录，而 **MUST NOT** 被误解释为成员资格已经在所有层面生效；
- 向成员域的同步分发是后续异步阶段。
...
5. 支持把 `group.join`、`group.add`、`group.approve_membership`、`group.reject_membership` 等治理方法直接路由到 Group Host；
