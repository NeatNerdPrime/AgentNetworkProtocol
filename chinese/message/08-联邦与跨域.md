...
以下 service-scoped getter / material 方法在跨域时 **MUST** 直接路由到最终公开 `ANPMessageService`，而不是通过私有中继包一层新协议：
...
- `group.e2ee.get_join_info`
  1. 解析 `body.group_did`
  2. 找到 Group Host 公开的 `ANPMessageService`
  3. 直接调用该服务
...
对于仍不是群成员的参与方，例如被邀请方、加入申请人或审批人：

- 与其相关的邀请通知、审批通知、审批结果通知 **SHOULD NOT** 通过群成员专用的 `group.incoming` 分发；
- 标准互通路径 **SHOULD** 使用 `group.governance_notice` 直接把治理通知发送到对应 Agent 所在域；
- 若部署方另用 `direct.send` 或其它私有机制承载同类通知，属于部署扩展，而不是 v1 标准跨域路径。
...
- 对 `group.incoming` 与 `group.state_changed`：

  caller anchor **MUST** 取 `body.group_did`

- 对 `group.governance_notice`：

  caller anchor **MUST** 取 `body.notice.group_did`
...
`attachment.get_download_ticket` 的协议级最终目标 **MUST** 是由附件清单 `access_info.control_service_did` 标识的公开 `ANPMessageService`。该公开服务入口可以在内部再路由到真正处理对象控制逻辑的 Object Service，但这属于实现细节。

因此，标准互通语义中的“最终处理端点”是：

- 协议级：`control_service_did` 对应的公开 `ANPMessageService`
- 实现级：该服务背后的内部 Object Service（可选）

调用方 **MUST NOT** 仅凭 `object_uri` 的 URL 域名去猜测控制面服务。
...
请求方 Agent 直接调用对象控制服务对应的公开 `ANPMessageService`。

该模式 **不属于 v1 MTI**；若部署启用，必须自行解决：

- Agent 如何获得并验证目标 `serviceDid`
- 客户端如何执行外层服务认证
- 客户端如何统一处理限流、重试和审计策略
...
6. 支持 `direct.e2ee.get_prekey_bundle`、`group.e2ee.get_key_package`、`group.e2ee.get_join_info` 的跨域路由；
7. 把 `attachment.get_download_ticket` 送到 `control_service_did` 标识的公开控制服务；
8. 明确禁止对象字节通过跨域服务调用链路常规转发；
9. 至少支持一种已排序群事件向成员域分发的机制，并使用 `group.governance_notice` 作为非成员治理通知的标准跨域路径；
10. 为参与跨域调用的 `ANPMessageService` 声明 `serviceDid`，并按方法级 caller anchor 校验外层 HTTP Message Signatures；
