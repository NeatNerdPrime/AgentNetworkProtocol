...
- 群 Base 跨域直接使用 `group.create`、`group.join`、`group.add`、`group.remove`、`group.leave`、`group.update_profile`、`group.update_policy`、`group.send`；
- 群 E2EE 跨域直接使用 `group.e2ee.create`、`group.e2ee.add`、`group.e2ee.remove`、`group.e2ee.send`；
...
8. 把 `attachment.get_download_ticket` 送到 `control_service_did` 标识的公开控制服务；
