...
# ANP Profile 5：Direct E2EE Profile（完整草案）
...
3. 至少有一个 `ANPKeyService` 或等价服务入口；
4. `ANPKeyService` 可被调用方解析和访问。
...
### 4.4 `ANPKeyService` 的角色

`ANPKeyService` **MUST** 承担：
...
7. 若存在 `one_time_prekey`，记录其 `key_id` 以供后续消费与审计。
...
此方法用于把新的 Prekey Bundle 发布到 `ANPKeyService`。
...
`ANPKeyService` 在返回 Bundle 时：

- **SHOULD** 优先返回仍在有效期内的最新 SPK；
- **SHOULD** 尽可能附带一个 OPK；
- 返回 OPK 后 **SHOULD** 将其标记为已分配；
- 若实现支持强一次性消费，则目标接收方成功处理 init 后 **MUST** 删除对应 OPK 私钥；
- 若目标服务拒绝或处理失败，是否允许 OPK 回收由部署策略决定，但 **SHOULD NOT** 导致同一 OPK 被并发重复分配。
...
若 Bundle 中 **没有** OPK，则发送方 **MUST** 计算：
...
若 Bundle 中 **有** OPK，则发送方 **MUST** 额外计算：
...
3. 从本地 Bundle 存储中定位 `SPK_B` 与 `OPK_B`；
...
1. 解析 DID 文档中的 `authentication`、`assertionMethod`、`keyAgreement`、`ANPKeyService`；
2. 发布与获取 `prekey_bundle`；
3. 验证 Bundle `proof`；
...
2. Bundle 绑定复用 `did:wba` proof，而不是原始 XEdDSA 专用格式；
