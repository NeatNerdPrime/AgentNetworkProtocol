...
当接收方收到的 `ratchet_header.dh_pub_b64u` 与当前记录的 `DHr` 不一致时，**MUST** 执行一次 DH ratchet：

1. 设 `DHr_new = ratchet_header.dh_pub_b64u`
2. 使用当前本地发送私钥与 `DHr_new` 计算：
   - `(RK, CKr) = kdf_rk(RK, DH(DHs, DHr_new))`
3. 生成新的本地发送 DH 密钥对 `DHs_new`
4. 再计算：
...
- 发起方 **MUST** 把 `sender_ephemeral_pub_b64u` 作为其初始 `DHs`
- 响应方 **MUST** 把 `sender_ephemeral_pub_b64u` 记录为初始 `DHr`
- 发起方从 `CK0` 开始其首条发送链
- 响应方把同一 `CK0` 作为首条接收链的起点，并在发出首条回复前执行一次新的 DH ratchet
...
若实现需要显式会话控制，可使用以下扩展对象；该能力不是 v1 最小互通要求：
...
6. 通过 `direct.send` 承载 `application/anp-direct-init+json`；
7. 通过 `direct.send` 承载 `application/anp-direct-cipher+json`；
8. 固定使用本 Profile 定义的 `kdf_rk`、`kdf_ck` 与派生 nonce 规则；
