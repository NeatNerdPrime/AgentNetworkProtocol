...
补充说明：在线协议里，前者通常通过 `proof.verificationMethod` 被引用，后者通常通过 `static_key_agreement_id`、`sender_static_key_agreement_id` 或 `recipient_static_key_agreement_id` 被引用；实现方不应让同一把密钥同时承担这两类语义。
...
- `proof.type`：证明对象的类型或 cryptosuite 标识，用于指示接收方按哪种证明语法解析。
...
此方法用于把新的静态 Prekey Bundle 发布到发送方 DID 公开的 `ANPMessageService`，并可选补充一批 OPK。
...
- 调用方 **MUST** 通过已认证会话或等价 hop / service 级认证访问该方法；v1 **不**要求额外定义新的业务 proof
...
- `meta.target` 把本方法明确声明为 service-scoped，而不是 endpoint-local；
...
- `published = true`：表示服务端已接受并保存本次发布，不表示该 Bundle 已被任何对端使用。
- `owner_did`：回显本次发布所归属的 Agent DID。
- `bundle_id`：回显当前已生效的静态 Bundle 标识，便于调用方缓存和审计。
- `published_at`：服务端确认写入该 Bundle 的时间。

成功响应 **MAY** 包含：

- `published_opk_count`

若返回 `published_opk_count`，其含义是本次成功写入或更新的 OPK 数量。
...
- `KA_B`：接收方长期静态 X25519 `keyAgreement` 公钥 / 私钥对。在线协议中它由 `recipient_static_key_agreement_id` 指明。
...
- `MK0` 用于加密或解密 `direct_init.ciphertext_b64u`
- `NONCE0` 用于该首条 init 密文的 AEAD nonce
- `CK1` 成为后续链状态的起点

v1 统一规定：`ciphertext_b64u` **只包含 AEAD ciphertext（含认证标签）本体**，不额外拼接或前置 nonce；nonce 一律按本 Profile 的 KDF 规则派生。
...
- `bundle_id`：绑定接收方所使用的具体 Bundle 版本，防止不同 Bundle 间的材料混用。
- `sender_static_key_agreement_id` / `recipient_static_key_agreement_id`：绑定双方实际参与 DH 的长期静态密钥。
...
- `recipient_bundle_id`：表示本次 init 使用了接收方哪一个静态 Bundle 版本；接收方可据此定位对应的 SPK 生命周期和重放范围。
- `recipient_static_key_agreement_id`：指向接收方 DID 文档中被本次建链使用的长期静态 `keyAgreement` 键。
- `recipient_signed_prekey_id`：指向接收方本次使用的 SPK 记录，接收方需据此找到正确的 SPK 私钥。
...
- `ciphertext_b64u`：使用 init 派生的首个消息密钥加密得到的密文字节；解密时必须结合 `AD_init`。
...
### 10.1.1 Ratchet KDF 定义

为保证跨实现对齐，v1 把 Root Key / Chain Key / Message Key / Nonce 的派生规则固定如下。
...
- `RK'`：新的 Root Key
- `CK`：由本次 DH ratchet 推进得到的新接收链或发送链的初始 Chain Key
...
- `CK'`：下一条链密钥
- `MK`：当前消息密钥
- `NONCE`：当前消息 AEAD nonce（12 bytes）

#### 规则

- 发送链与接收链 **MUST** 都使用同一 `kdf_ck` 规则；
- 所有后续 `ciphertext_b64u` 字段 **MUST** 只携带 AEAD ciphertext，本规范不允许把 nonce 作为独立线协议字段附加到 `application/anp-direct-cipher+json` 中；
- 若实现维护 skipped keys，缓存对象 **SHOULD** 同时缓存对应 `MK` 与 `NONCE`，避免重复派生歧义。
...
- `suite`：指出该密文应按哪一个套件解释；通常它应与会话建立时的套件一致。
- `ratchet_header`：承载后续消息公开可见但必须受认证绑定的 ratchet 坐标。
- `ciphertext_b64u`：对内层 `Application Plaintext` 加密后的 AEAD ciphertext 字节串，使用无填充 base64url 表示；该字段 **不**包含独立 nonce，nonce 由 `kdf_ck` 派生。
...
- `annotations`：加密保护下的扩展应用元数据；它与 P3 的 `annotations` 语义保持一致，不应用于外层路由或幂等判定。
...
`application_content_type` 保留在内层 `Application Plaintext` 中，由 AEAD 本身保护其完整性；v1 **不**要求在解密前把它作为外层 AAD 参与认证。
...
但这些控制对象属于**非 MTI 扩展**。v1 的推荐重建路径仍是“直接发送新的 `application/anp-direct-init+json` 重新建链”，而不是依赖额外的控制对象互通。
...
3. 对 `direct.e2ee.publish_prekey_bundle` 与 `direct.e2ee.get_prekey_bundle` 使用 service-scoped `meta.target` 规则；
4. 验证 Bundle `proof`；
5. 使用 MTI 套件执行 X3DH-like 初始建链；
6. 固定实现 `kdf_rk`、`kdf_ck` 与基于 `kdf_ck` 的 nonce 派生规则；
7. 通过 `direct.send` 承载 `application/anp-direct-init+json`；
8. 通过 `direct.send` 承载 `application/anp-direct-cipher+json`；
9. 维护 Double Ratchet-like 状态；
10. 实现 `MAX_SKIP` 与 `MKSKIPPED`；
11. 防止初始消息重放导致的密钥灾难性复用；
12. 不把设备或内部副本概念暴露到线协议中。
