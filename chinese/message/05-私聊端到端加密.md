...
- v1 标准路径下，`params.auth` **SHOULD** 缺省；
...
发起方 A 在收到同一 `session_id` 下的首条有效回复时，若当前会话 `status = "pending-confirmation"`，则 **MUST** 按以下顺序处理：
...
9. 固定使用本 Profile 定义的 `kdf_rk`、`kdf_ck` 与派生 nonce 规则；
10. 对 `Signed Bundle Object`、`AD_init`、`AD_msg` 与 `Application Plaintext` 固定使用 UTF-8 + RFC 8785 JCS；
11. init 消息消耗链上的第 0 条消息，后续链状态 **MUST** 从 `CK1` 开始；
12. 发起方在收到对端第一条有效回复前 **MUST NOT** 发送独立的 `application/anp-direct-cipher+json`；
13. 维护 Double Ratchet-like 状态；
14. 实现 `MAX_SKIP` 与 `MKSKIPPED`；
15. 区分外层幂等重试与密码学 replay，并防止初始消息重放导致的灾难性密钥复用；
16. 不把设备或内部副本概念暴露到线协议中。
