...
- `ciphertext_b64u`：对内层 `Application Plaintext` 加密后的字节串，使用无填充 base64url 表示。
...
- `attributes`：加密保护下的扩展应用元数据；它不同于外层 `meta`，不应用于外层路由或幂等判定。
...
- `application_content_type`：绑定内层原始负载类型，防止同一密文被解释成不同内容类型。
...
若存在 `attachment_manifest_digest`，其用途是把附件清单摘要与密文绑定，降低“替换附件清单但保留同一外层消息”的风险。
