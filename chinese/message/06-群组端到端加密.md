...
`group_key_package` 主要用于由现有成员或治理服务代他人执行的 `group.add` 路径。除非部署显式声明支持 last-resort 语义，否则同一个 `group_key_package` **SHOULD** 只用于一次成功的加人流程；服务端 **SHOULD** 避免把同一个有效 KeyPackage 并发发放给多个会成功提交的加人请求。
...
- 若邀请接受之后还需要目标成员自助完成密码学入群收尾，则该后续步骤 **SHOULD** 使用携带已批准 `request_id` 的 `group.join`，而不是再次直接消费 `invitation_id`。
...
2. 基于已批准 `request_id` 的自助式密码学入群收尾。
...
当该请求用于立即或审批后入群收尾时，`e2ee.handshake` **MUST** 存在。若该请求不是新建 join-request，而是批准后的自助式入群收尾，则 `request_id` **MUST** 指向一个已批准的 `membership_request`。
...
在 `group-e2ee` 模式下，`group.add` 用于由现有成员或治理服务代他人完成密码学加人。
...
- 若群策略要求被加入方先显式接受，则 Group Host **MUST** 拒绝 `group.add`，并 **SHOULD** 要求调用方改用 `group.invite` / `group.accept_invite` 路径；若后续仍需目标成员自助完成密码学入群收尾，则该步骤 **SHOULD** 使用带已批准 `request_id` 的 `group.join`。
