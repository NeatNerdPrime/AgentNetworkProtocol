...
对默认 `e1_` did:wba，`did_wba_binding.proof` **MUST** 使用 `DataIntegrityProof`，且 `cryptosuite` **MUST** 为 `eddsa-jcs-2022`。`did_wba_binding` 的生成与验证 **MUST** 以“移除 `proof` 后的整个 `did_wba_binding` 对象”为被保护文档，并 **MUST** 遵循 did:wba 文档 proof 所要求的标准算法流程。
...
`group_cipher_object` 是 `group.e2ee.send` 的协议消息体对象。
...
- `notice_type = "welcome-delivery"` 时，`welcome_b64u` **MUST** 存在；
...
本章给出新架构下的密码学机制说明。  
它不是新的 JSON-RPC 方法，而是对 `group.e2ee.create`、`group.e2ee.add`、`group.e2ee.remove`、`group.e2ee.send` 背后 MLS 语义的规范说明。

### 11.2 `group.e2ee.create` 的密码学语义

创建群时，owner 的本地 MLS runtime **MUST**：

1. 验证 `creator_key_package`
2. 创建新的 MLS group state
3. 生成新的 `crypto_group_id`
4. 把 owner 作为第一个 MLS 成员加入
5. 形成初始 `epoch`

这里的“密钥协商”不是两人建链，而是：

- 用 MLS 的初始群上下文创建第一代群状态
- 建立后续成员变更和消息加密所需的共同群状态基础

### 11.3 `group.e2ee.add` 的密码学语义
...
### 11.4 `group.e2ee.remove` 的密码学语义

执行 `group.e2ee.remove` 时，owner 的本地 MLS runtime **MUST**：

1. 基于当前群状态执行 MLS `Remove`
2. 生成新的 `Commit`
3. 推进新的 `epoch`
4. 使被移除成员失去后续消息解密能力

因此，移除不仅是业务状态变化，也是一次密钥轮换。

### 11.5 `group.e2ee.send` 的加密语义

发送群消息时，发送者本地 MLS runtime **MUST**：

1. 构造 `Group Application Plaintext`
2. 按本 Profile 定义的 `authenticated_data` 绑定上下文
3. 使用当前群状态把内层明文加密成 MLS `PrivateMessage`
4. 生成 `group_cipher_object`
5. 把该对象作为 `group.e2ee.send` 的 `body` 提交给 Group Host

### 11.6 群消息解密语义

接收成员在收到群消息后，**MUST**：

1. 根据 `group_did` 与本地状态找到正确的 MLS 群状态
2. 校验 `crypto_group_id_b64u` 与本地绑定一致
3. 校验 `epoch` 是否可接受
4. 使用对应 MLS 状态解密 `private_message_b64u`
5. 验证 `authenticated_data`
6. 解析出内层 `Group Application Plaintext`

若任一步骤失败，接收方 **MUST NOT** 将该消息作为有效群消息交付上层。

### 11.7 Welcome 处理是本地行为，不是协议方法

本方案不定义 `accept_welcome` 方法。  
新成员在收到 `welcome-delivery` Notice 后，本地 MLS runtime **MUST**：

1. 使用 `welcome_b64u` 初始化本地 MLS 群状态
2. 绑定 `group_did`、`group_state_ref`、`crypto_group_id`、`epoch`
3. 准备接收后续群消息与 `commit-delivery`

这是一段**本地行为规范**，不是新的 JSON-RPC 协议方法。
...
### 16.2 `actor_proof` 与 MLS 成员签名的关系
...
7. `group.e2ee.send`
8. did:wba 绑定验证
9. `group.e2ee.create` 的 service-scoped 目标模型
10. `group.e2ee.add/remove/send` 的 group-addressed 目标模型
11. `group.e2ee.notice` 的 agent-addressed 通知模型
12. owner 作为唯一 MLS 控制者
13. 通过 P4 业务状态驱动 `create/add/remove`
14. `group.e2ee.send` 直接发送 MLS 密文，不再经 `group.send` 包装
15. `group.e2ee.notice` 用于 `welcome-delivery` 与 `commit-delivery`
16. `group.incoming` 继续作为群消息接收通知
17. 只有消息面进入 `PrivateMessage`
18. `group_receipt`、`group_state_version`、`group_event_seq` 的业务语义与 P4 一致
