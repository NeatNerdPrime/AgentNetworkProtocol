...
通过目标群 DID 暴露的 `ANPMessageService` 为被邀请方或主动加入方获取外部加入所需的 `group_join_info`。
...
在 `group-e2ee` 模式下，`group.accept_invite` **默认表示治理上的接受邀请**。
...
当该请求用于立即或审批后入群收尾时，`e2ee.handshake` **MUST** 存在。
...
- 若群策略要求被加入方先显式接受，则 Group Host **MUST** 拒绝 `group.add`，并 **SHOULD** 要求调用方改用 `group.invite` / `group.accept_invite` / `group.join` 路径。
