...
对 `group.join`、`group.accept_invite`、`group.approve_membership` 等 onboarding / bootstrap 方法，是否允许先以 `transport-protected` 进入治理流程，取决于 P4 中的 `bootstrap_security_profile`；若该字段要求 `group-e2ee`，则这些路径在进入生效阶段前也 **MUST** 满足本 Profile 约束。
...
### 7.6.1 `Group Application Plaintext`

所有群应用消息在进入 MLS `PrivateMessage` 前 **MUST** 先归一化为内层群应用明文对象：
...
- P4 中 `group.send.body.thread_id`、`reply_to_message_id`、`annotations` 在 `group-e2ee` 下 **MUST** 移入该内层对象；
- 外层 `meta.content_type` **MUST NOT** 继续表示这些原始应用类型。

### 7.6.2 Group E2EE Notification Mapping

为使 P4 的通知体系与本 Profile 闭环，`group-e2ee` 下的三类通知承载规则如下：

1. **`group.incoming`**
   - `body.group_did`、`body.message_id`、`body.accepted_at` **MUST** 存在；
   - `body.cipher` **MUST** 为 `group_cipher_object`；
   - `body.group_receipt` **SHOULD** 存在。

2. **`group.state_changed`**
   - `body` 继续承载 P4 的统一 event 对象；
   - event **MAY** 额外携带 `e2ee_state`，其中推荐包含：
     - `crypto_group_id_b64u`
     - `epoch`
     - `epoch_authenticator`

3. **`group.governance_notice`**
   - `body.notice` 继续承载 P4 的统一 notice 对象；
   - 当该 notice 需要向目标成员投递加密入群材料时，`body.notice.e2ee` **MAY** 包含：
     - `group_join_info`
     - `welcome_b64u`
     - `crypto_group_id_b64u`
     - `epoch`
...
上述映射只扩展 E2EE 承载字段，不改变 P4 中通知对象本身的语义边界。
...
由某 Agent 向其 DID 文档公开的 `ANPMessageService` 发布一个可用于群加入的 KeyPackage。
...
- `meta.target.did` **MUST** 等于 `meta.sender_did` 所公开 `ANPMessageService.serviceDid`
- 调用方 **MUST** 通过已认证会话或等价 hop / service 级认证访问该方法；v1 不额外定义新的业务 proof
...
#### 8.2.2 请求要求
- `meta.target.kind = "service"`
- `meta.target.did` **MUST** 等于 `body.target_did` 所公开 `ANPMessageService.serviceDid`
- 标准互通路径 **MUST** 至少要求 hop / service 级认证；匿名获取不属于 v1 MTI
...
通过目标群 DID 暴露的 `ANPMessageService` 为被邀请方或已获批准的主动加入方获取外部加入所需的 `group_join_info`。

#### 8.3.2 请求要求
- `meta.target.kind = "service"`
- `meta.target.did` **MUST** 等于目标 `group_did` 所公开 Group Host `ANPMessageService.serviceDid`
- 标准互通路径 **MUST** 至少要求 hop / service 级认证；匿名访问不属于 v1 MTI
...
对于 `group.invite`、`group.accept_invite`、`group.join`、`group.add`、`group.approve_membership`、`group.reject_membership` 这类成员治理方法，Group Host **MUST** 先判断治理前置条件是否已经满足，再决定是否推进 MLS 成员变更。
...
- `group.invite` 本身 **MUST NOT** 改变 MLS 成员集。
...
- 建立邀请记录；
- 指定预期角色；
- 为后续 `group.accept_invite` 提供依据。

### 9.4 `group.accept_invite`

在 `group-e2ee` 模式下，`group.accept_invite` 是邀请路径的标准接受动作，**默认表示治理上的接受邀请**。

- 若群策略允许受邀者在接受邀请后立即完成密码学入群，则该请求 **MAY** 携带 `e2ee.handshake`（External Commit 提交对象），并在同一请求中完成 `External Commit`；
- 若群策略仍要求管理员审批，或要求后续重新获取 fresh `group_join_info`，则 Group Host **MUST NOT** 处理该握手，并 **MUST** 仅记录邀请接受结果与待决治理状态。
- 若邀请接受之后还需要目标成员自助完成密码学入群收尾，则该后续步骤 **SHOULD** 使用携带已批准 `membership_request_id` 的 `group.join`，而不是再次直接消费 `invitation_id`。
...
`group.join` **MUST NOT** 直接替代 `group.accept_invite` 去消费 `invitation_id`。
...
- 若群策略要求被加入方先显式接受，则 Group Host **MUST** 拒绝 `group.add`，并 **SHOULD** 要求调用方改用 `group.invite` / `group.accept_invite` 路径；若后续仍需目标成员自助完成密码学入群收尾，则该步骤 **SHOULD** 使用带已批准 `membership_request_id` 的 `group.join`。
...
- 若目标尚未进入 MLS 成员集（例如仍处于 `invited`、`pending` 等阶段），则该请求 **MUST** 只改变治理状态或应用层成员状态，且 **MUST NOT** 推进 MLS `epoch`。
...
### 9.10 `group.leave`
在 MLS 1.0 主线约束下，`group.leave` 的 v1 语义采用“**应用层退出立即生效，密码学移除可后续完成**”的收敛模型。

当离群成员发送 `group.leave` 并被 Group Host 接受后，Group Host **MUST**：

- 在应用层把该成员状态更新为 P4 定义的 `left`；
- 为该离群动作分配新的 `group_event_seq` 与 `group_state_version`；
- 停止把后续新的群事件继续视为该成员的正常可见事件；
- 如仍需由剩余授权成员提交 Remove + Commit 完成密码学层移除，则把该后续工作视为实现或后续治理步骤，而不是新的标准成员状态。
...
- `leave_effective = true` 表示应用层成员关系已经结束；
- `crypto_removal_pending = true` 表示 MLS 成员集的最终移除仍待后续 Remove + Commit 完成，但这 **MUST NOT** 被建模为新的标准成员状态。

### 9.11 `group.update_profile`
...
请求 `body` **MUST** 直接为 `group_cipher_object`，并满足：

- 外层 `meta.content_type` **MUST** 固定为 `application/anp-group-cipher+json`；
- 外层 `body.text` / `body.payload` / `body.payload_b64u` **MUST NOT** 出现；
- 内层原始应用内容 **MUST** 封装进 MLS `PrivateMessage`；
- 原始应用内容类型 **MUST** 只通过内层 `Group Application Plaintext.application_content_type` 表达；
- `body.group_state_ref` **MUST** 与当前群状态绑定；
- 若发送方希望表达“基于哪一个群状态快照生成此消息”，则 **MUST** 使用 `body.group_state_ref.group_state_version`，而不是另设外层明文字段。
...
### 10.2 握手消息
会触发 Group Host 授权判断、排序或成员变更验证的握手提交对象，v1 **MUST** 使用 Host 可见的承载格式。

收敛规则如下：

- **群应用消息**：`group.send` **MUST** 使用 MLS `PrivateMessage`
- **会改变成员集、需要 Host 校验方法语义的握手提交**：**MUST** 使用 `mls-public-message` 或等价的 Host 可见承载格式
- “握手默认也走 `PrivateMessage`，仅必要时再退回可见格式” **不**属于 v1 MTI

对于仍处于治理待决阶段的 `group.accept_invite`、`group.join`、`group.add`、`group.approve_membership`：
...
为避免不同实现各自猜测，本 Profile 进一步固定下列最小映射：

| 业务方法 | 允许的最小 `handshake_kind` |
|---|---|
| `group.join`（立即加入或审批后自助完成） | `external-commit` |
| `group.accept_invite`（若同请求完成密码学入群） | `external-commit` |
| `group.add`（直接加人） | `commit` |
| `group.approve_membership`（批准即直接加人） | `commit` |
| `group.remove` | `commit` |
| `group.send` | 不适用；必须是 `PrivateMessage` 应用消息 |

若业务方法与握手类型、目标成员或引用材料不一致，Group Host **MUST** 拒绝请求。
...
7. `group.add`、`group.approve_membership`、`group.reject_membership`、`group.remove`、`group.send`
8. `group.accept_invite` 或 `group.join` 至少一种加密加入路径
9. 在治理前置条件未满足时，不推进 MLS 成员集
10. `group_receipt` 与 `group_state_version` / `group_event_seq` 绑定
11. `group.send` 使用 `PrivateMessage`
12. `group.send` 的外层 `meta.content_type` 固定为 `application/anp-group-cipher+json`，且 `body` 直接等于 `group_cipher_object`
