...
- **Sender Proof**：由发送方 Agent 基于 did:wba JSON 承载认证生成的应用层签名证明，用于证明某条 `direct.send` 确由 `sender_did` 对应私钥持有者发起。
- **Hop Authentication**：某一跳服务到服务、Agent 到服务之间的传输层/请求层身份认证。Hop Authentication 可以变化，但不得替代 Sender Proof。
- **Logical Target URI**：为使应用层签名能够跨中继保持稳定，由本 Profile 定义的逻辑目标 URI；它绑定 ANP 的业务目标，而不是某一跳的具体 HTTP URL。
...
- `auth.scheme` **MUST** 等于 `did:wba-json-v1`；
...
- 若 Signed Direct Payload 中包含 JSON 对象，发送方与接收方 **SHOULD** 使用 RFC 8785 JCS 进行规范化序列化；
...
11. `auth.scheme` **MUST** 等于 `did:wba-json-v1`
