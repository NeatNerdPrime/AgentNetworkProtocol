...
在 Direct Base 中，`meta.content_type` 表示当前 `body` 实际承载的应用对象类型。本 Profile 最小互通 **MUST** 支持以下内容类型：
...
- `params.auth.sender_proof` 若存在，**MUST** 为原始 `sender_proof` 的无损副本，而不是中间服务重写后的新证明
...
5. 检查 `keyid` 所属 DID 是否与 `meta.sender_did` 完全一致；
6. 检查 `keyid` 指向的验证方法是否存在且被 DID 文档 `authentication` 关系授权；
7. 若 `meta.sender_did` 为路径型 `e1_` DID，则继续执行：
   - 验证 DID Document 顶层 `proof`；
   - 读取 `proof.verificationMethod`；
   - 使用该公钥重算 DID 路径中的 `e1_` thumbprint；
   - 验证 `proof.verificationMethod`、DID 路径指纹与 DID 文档绑定一致；
8. 检查 `created` / `expires` 是否在允许窗口内；
9. 检查 `nonce`、`operation_id`、`message_id` 的重放防护策略；
10. 若任一步骤失败，**MUST** 拒绝请求。

实现 **MUST NOT** 省略第 7 步中的 `e1_` 绑定检查；否则容易把“文档里碰巧还有另一把可验证签名的 key”误当成“该 DID 自身绑定成立”。
...
### 9.5 Access Token 优化
本节是**非规范性优化建议**，不属于 P3 的最小互通要求。

基于 did:wba 的 access token 流程 **MAY** 用于优化重复请求，但对 `direct.send` 而言：

- access token **MUST NOT** 替代 `auth.sender_proof` 作为跨域原发者证明；
- sender-constrained access token **SHOULD** 优先于普通 Bearer token；
- 是否启用 token、token 如何签发、是否与连接或服务身份绑定，均属于部署优化，而不是私聊基础语义的 MTI 组成部分。
