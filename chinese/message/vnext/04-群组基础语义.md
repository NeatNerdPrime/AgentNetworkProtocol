...
6. 若 `subject_method = "group.remove"`，则 `event_type` **MUST** 为 `member-removed`，且 `membership_status`（若存在）**MUST** 为 `removed`；
7. 若 `subject_method = "group.leave"`，则 `event_type` **MUST** 为 `member-left`，且 `membership_status`（若存在）**MUST** 为 `left`。
...
21. 向事件 `subject_did` 的一次自身作用域 `group.state_changed` 最终投递（`member-removed` 与 `member-left`），在有界重试与保留上限内尝试，且绝不作为状态变化生效的前提
