...
- `provider_asserted`：did:wba Provider 提供并通过 03-did:wba 方法规范 2.5.6 节验证的 `providerTransitionAssertion`，但没有旧 binding key 或预授权 recovery key proof；仅有 TLS、`successorDid`、Handle 或 409 hint 不满足该等级；
- `unverified`：不存在可接受的连续性证据。

迁移 resolver 和验证层 **MUST NOT** 自行依据 `provider_asserted` 合并身份或作出高信任业务授权决定；它只把该 assurance 原样报告给所属业务层。基于 `provider_asserted` 的自动继承，只能来自业务系统显式接受 provider 被攻破风险与账户恢复需求之间取舍的策略，而不能因为该声明被提升成了密码学连续性。
