...
本 Profile 按 [ANP-02 身份输入](../../vnext/02-ANP-基于DID的身份认证协议.md#identity-input)及适用 DID 方法绑定解析、验证 DID 文档。`did:wba`、`did:web` 及其他已支持方法使用本 Profile 相同的服务发现、密钥用途和设备资格规则；方法特有的文档验证由对应方法规范定义。
...
本草案首个注册的自动迁移路径，是最后一段采用 `e1_` 指纹 Profile 的路径型 did:wba DID。解析器 **MUST** 从此前可信的 DID 出发，按 [ANP-03 方法规则](../../vnext/03-did-wba方法规范.md#wba-method-rules)验证每一跳及最终活动 DID 文档，并拒绝循环、冲突后继和可信缓存冲突。
