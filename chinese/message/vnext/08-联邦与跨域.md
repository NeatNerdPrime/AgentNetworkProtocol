...
若在 JSON-RPC 应用处理前发现 DID 已停用，did:wba HTTP 层可以返回其方法规范定义的 HTTP `409`。若 ANP 方法已进入应用处理，且路由、请求正文或已保存状态中的 DID 已被替代，最终业务服务返回 P1 `anp.did_superseded`。同一请求 **MUST NOT** 同时返回两层错误。
