...
6. 接收方通过 `attachment.get_download_ticket` 获取下载票据；
...
sequenceDiagram
    ...
    A->>AS: attachment.commit_object
    AS-->>A: committed = true
    A->>GH: group.send (group-e2ee, 内层 manifest 含 object_key)
    GH-->>A: accepted + group_receipt
    GH-->>M: group.incoming
    M->>M: 解出内层 manifest\n得到 object_key + nonce
    M->>AS: attachment.get_download_ticket
...
- `access_info`：对象下载定位与控制面发现信息；**MUST**
...
3. `control_service_did` **MUST** 存在，且 **MUST** 等于负责该对象控制面方法的公开 `ANPMessageService.serviceDid`
4. 接收方下载前 **MUST** 先调用 `attachment.get_download_ticket`
5. 接收方下载时 **MUST** 对 `object_uri` 发起 `GET`，并在 `Authorization` 头中携带票据
6. 调用方 **MUST NOT** 仅根据 `object_uri` 的 URL 域名猜测控制面服务；**MUST** 以 `control_service_did` 为发现锚点
...
若 `control_service_did` 背后的实现不是发送方服务本身，部署方 **MUST** 保证该控制服务能以本地状态或内部同步方式拿到所需的 Access Grant。
...
- v1 **不要求** 在 Object 控制服务端再次直接验证独立终端的 `sender_proof` 或 `actor_proof`
...
发送方后续构造 `attachment_manifest.access_info` 时，`object_uri` 与 `control_service_did` **SHOULD** 直接取自 `attachment.create_slot` 或 `attachment.commit_object` 的成功响应
