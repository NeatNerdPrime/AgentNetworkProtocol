...
- 说明：
  ...
  - `kind = "group"` 表示目标为群组；
  - `kind = "service"` 表示目标为某个服务身份 DID（通常对应 P2 中 `ANPMessageService.serviceDid`）；
...
- 规则：
  ...
  - 若某 Profile 定义了 `target_scope`，则服务端 **MUST** 至少基于 `(sender_did, target_scope, method, operation_id)` 进行幂等判定；
...
- 说明：
  ...
  - `message_id` 与 `operation_id` 可以相同，也可以不同；若不同，必须在实现中保持稳定映射；
  - `message_id` **MUST NOT** 隐式替代 `operation_id` 充当操作级幂等键。
...
实现方 **MAY** 返回更细粒度的能力扩展字段（如方法级能力、Notification 能力、认证 scheme 能力），但这些扩展字段不影响本节定义的最小标准响应结构。
...
- 它与 P2 DID 文档中的 `ANPMessageService.serviceDid` 表达同一服务身份，但运行时结果字段使用 snake_case。
- 若实现方需要返回更细粒度能力（如方法级能力、Notification 能力或认证 scheme），**MAY** 在该最小结构之上追加扩展字段。

### 8.3 协商规则

- DID 文档中的静态扩展字段（如 `profiles`、`securityProfiles`、`authSchemes`）仅表示可缓存的静态 hint；
...
- 对未知 `meta` 字段，接收方 **MUST** 拒绝；
- 对未知 `auth` 字段，接收方 **MUST** 拒绝；
