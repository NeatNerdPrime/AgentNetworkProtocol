...
- 说明：
  ...
  - 未明确声明的 service 控制面方法，接收方 **MUST** 视为必须显式提供 `target`。

#### 6.2.5.1 目标建模模式声明规则

为降低跨实现歧义，ANP v1 把目标建模收敛为四类：

1. **agent-addressed**  
   - `meta.target.kind = "agent"`
   - `meta.target.did` **MUST** 为目标 `agent_did`
...
3. **service-scoped**  
   ...
   - `meta.target.did` **MUST** 为目标公开 `ANPMessageService.serviceDid`
   - 典型场景包括：能力查询、密钥材料方法、附件控制面方法等

4. **endpoint-local**  
   - 仅当具体 Profile 明确声明该方法是 endpoint-local 时，才允许省略 `meta.target`
   - endpoint-local 不等于“实现自己猜目标”；其绑定对象 **MUST** 由该 Profile 在其它字段中写清楚

若一个方法既不是 endpoint-local，又没有携带与声明模式一致的 `target`，接收方 **MUST** 返回 `anp.invalid_target_binding`。
...
- 说明：
  - `meta.content_type` **MUST** 始终描述当前线协议 `body` 所承载的实际对象类型；
  ...
  - 本字段的外层语义由 P1 唯一规定；其它 Profile **MUST NOT** 重写“外层 `content_type` 表示什么”的规则。
- 示例：
  - `"text/plain"`
  - `"application/json"`
...
- Group E2EE 附件消息：外层 `meta.content_type = "application/anp-group-cipher+json"`，内层 `application_content_type = "application/anp-attachment-manifest+json"`
...
| 1014 | `anp.invalid_target_binding` | 方法声明的目标模式与 `meta.target` 不匹配 |
