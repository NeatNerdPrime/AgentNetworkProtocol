...
- **Message-bearing Notification**：作为 Notification 承载应用层消息负载的推送方法，例如 `direct.incoming`、`group.incoming`。
...
本附录定义 P3、P4 以及后续业务 Profile 共享的应用层原发者证明（Origin Proof）承载约定。

### A.1 共享 `auth.scheme`

当某业务 Profile 使用基于 RFC 9421 风格组件映射的应用层原发者证明时：

- `auth.scheme` **MUST** 等于 `anp-rfc9421-origin-proof-v1`
- 具体业务 Profile **MAY** 使用不同字段名承载具体 proof，例如：
  - P3：`auth.sender_proof`
  - P4：`auth.actor_proof`
- 字段名不同 **MUST NOT** 改变该 scheme 的共享语义

### A.2 共享 proof 对象结构

具体 proof 对象 **MUST** 至少包含以下字段：
...
- 具体业务 Profile **MUST** 定义唯一的 Signed Payload 结构；
- `contentDigest` **MUST** 绑定一个不包含 `auth` 的业务对象；
- Signed Payload **MUST** 使用 RFC 8785 JCS 进行规范化序列化；
- `auth` 本身 **MUST NOT** 进入 `contentDigest`；
- 上游服务路由信息、本地缓存字段、运维追踪字段 **MUST NOT** 进入 Signed Payload。

### A.4 签名组件映射职责分离

本附录只定义共享的 proof 承载结构，不直接定义每个业务 Profile 的签名组件映射。

因此：

- `@method` 的业务映射 **MUST** 由对应业务 Profile 定义；
- `@target-uri` 的逻辑目标 URI 规则 **MUST** 由对应业务 Profile 定义；
- `content-digest` **MUST** 绑定该业务 Profile 所定义的 Signed Payload。
...
- 具体 did:wba 校验规则 **MUST** 遵循 did:wba 方法规范及对应业务 Profile 的验证步骤。
