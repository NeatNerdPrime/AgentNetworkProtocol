...
1. `meta.content_type` **MUST** 始终描述当前线协议 `body` 所承载的实际对象类型；
2. 若某安全 Overlay 在 `body` 中承载密文 envelope，则此处 **MUST** 表示密文 envelope 的类型；
3. 原始应用内容类型若与外层承载对象类型不同，**MUST** 由对应 Overlay 在其内部字段（如 `application_content_type`）中表达；
4. 对外层 `meta.content_type` 的解释权由本 Profile 唯一规定；其它业务 Profile 与安全 Overlay **MUST NOT** 重写该字段的外层语义。
...
- Group E2EE 附件消息：`application/anp-group-cipher+json`
