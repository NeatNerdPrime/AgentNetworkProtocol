...
DID 持有者在其 DID Document 的 `service` 中添加 `ANPHandleService` 类型的条目，声明其关联的 Handle：
...
`ANPHandleService` 用于表达 DID 持有者对其 Handle 绑定关系的反向声明。

在本规范 v1 中，`ANPHandleService.serviceEndpoint` 的主要作用是声明该 DID 所使用的 Handle Provider domain。它不是 Profile 页面入口，也不是消息服务端点，而是名称绑定校验所使用的域名声明。

验证者在 v1 中只比较 `ANPHandleService.serviceEndpoint` 的 domain 是否与输入 Handle 的 domain 一致，不要求 path 完全一致。为保持实现简洁与可读性，`serviceEndpoint` 可以使用对应 Handle 的 Resolution Endpoint，也可以使用同一域名下的其他稳定 HTTPS URL；但其 domain 必须（MUST）与 Handle 的 domain 一致。
...
sequenceDiagram
    ...
    D-->>V: DID Document
    Note over V: 3. 检查 DID Document 中<br/>ANPHandleService 的 domain 是否与 Handle 的 domain 一致
    alt 双向一致
        Note over V: ✓ 验证通过
    else 不一致
...
### 6.3.1 ANPHandleService 的使用规则（v1）
...
8. 若不存在 `ANPHandleService`，或 `serviceEndpoint` 的 domain 不一致，则该 Handle 绑定不得视为已验证绑定。
...
在本规范 v1 中，验证者只使用 `ANPHandleService.serviceEndpoint` 的 domain 部分进行反向绑定校验，不要求 path 完全一致。
...
- DID 持有者需要更新 DID Document 中的 `ANPHandleService`，以继续声明其所使用的 Name Service domain
...
2. DID 持有者在 DID Document 中通过 `ANPHandleService` 反向声明 Handle 所属的 Name Service domain；
...
15. 在本规范 v1 中，执行双向绑定校验时，验证者必须比较 `ANPHandleService.serviceEndpoint` 的 domain 与输入 Handle 的 domain 是否一致，不要求 path 完全一致
...
10. DID 持有者在 Handle Provider 迁移后应更新 DID Document 中的 `ANPHandleService`
