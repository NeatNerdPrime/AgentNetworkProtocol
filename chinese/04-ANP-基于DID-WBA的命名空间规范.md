...
- `serviceEndpoint`：指向 Handle Provider 域下的 HTTPS 端点。在本规范 v1 中，验证者仅使用该 URL 的 domain 部分进行反向绑定校验。

`HandleService` 用于表达 DID 持有者对其 Handle 绑定关系的反向声明。

在本规范 v1 中，`HandleService.serviceEndpoint` 的主要作用是声明该 DID 所使用的 Handle Provider domain。它不是 Profile 页面入口，也不是消息服务端点，而是名称绑定校验所使用的域名声明。

验证者在 v1 中只比较 `HandleService.serviceEndpoint` 的 domain 是否与输入 Handle 的 domain 一致，不要求 path 完全一致。为保持实现简洁与可读性，`serviceEndpoint` 可以使用对应 Handle 的 Resolution Endpoint，也可以使用同一域名下的其他稳定 HTTPS URL；但其 domain 必须（MUST）与 Handle 的 domain 一致。

后续版本可以在保持兼容的前提下，引入 `providerDid` 等更强的 Name Service 提供者标识方式，以支持更明确的 provider 身份表达与迁移验证。
...
sequenceDiagram
    ...
    D-->>V: DID Document
    Note over V: 3. 检查 DID Document 中<br/>HandleService 的 domain 是否与 Handle 的 domain 一致
    alt 双向一致
        Note over V: ✓ 验证通过
    else 不一致
...
4. 提取该条目 `serviceEndpoint` 的 domain，并与输入 Handle 的 domain 进行比较

若 domain 一致，则绑定关系可信；否则必须（MUST）视为不可信。实现者可以（MAY）进一步向用户告警。

### 6.3.1 HandleService 的使用规则（v1）

验证者在执行双向绑定校验时，应按以下规则使用 `HandleService.serviceEndpoint`：

1. 根据输入 Handle 提取其 domain；
2. 通过 Handle Resolution Endpoint 解析该 Handle，并提取其中的 `did`；
3. 解析该 `did` 对应的 DID Document；
4. 在 DID Document 的 `service` 中查找 `type = "HandleService"` 的条目；
5. 提取该条目 `serviceEndpoint` 的 URL scheme 与 domain；
6. 验证 `serviceEndpoint` 使用 `https`，并比较其 domain 是否与步骤 1 中的 Handle domain 一致；
7. 若一致，则说明 DID 持有者接受该 Handle 所属 domain 提供的名称绑定关系；
8. 若不存在 `HandleService`，或 `serviceEndpoint` 的 domain 不一致，则该 Handle 绑定不得视为已验证绑定。

在本规范 v1 中，反向绑定校验只比较 domain，不要求 `serviceEndpoint` 的 path 与某个具体 Resolution Endpoint 完全一致。后续版本可以引入 `providerDid`，在保持兼容的前提下实现更强的 Name Service 提供者身份验证。

对于安全敏感场景（如身份认证、授权决策、消息发送前收件人确认），验证者必须（MUST）执行上述校验。
...
在本规范 v1 中，验证者只使用 `HandleService.serviceEndpoint` 的 domain 部分进行反向绑定校验，不要求 path 完全一致。
...
- DID 持有者需要更新 DID Document 中的 `HandleService`，以继续声明其所使用的 Name Service domain
- 客户端在新地址解析到结果后，必须（MUST）重新执行双向绑定验证，不得仅凭重定向接受新的绑定关系
- 如果未来需要在不改变当前 v1 互操作模式的前提下表达更强的 provider 身份，可在后续版本引入 `providerDid` 机制
...
1. Handle Provider 正向声明 Handle → DID；
2. DID 持有者在 DID Document 中通过 `HandleService` 反向声明 Handle 所属的 Name Service domain；
3. 验证者在本规范 v1 中检查两个方向的 domain 一致性；
4. 后续版本可以进一步引入 `providerDid`，提供更强的 Name Service 提供者身份验证能力。
...
3. Alice 更新新 DID Document 中的 `HandleService`
...
14. 在身份认证、授权决策、即时消息收件人解析及其他安全敏感场景中，验证者必须执行双向绑定验证
15. 在本规范 v1 中，执行双向绑定校验时，验证者必须比较 `HandleService.serviceEndpoint` 的 domain 与输入 Handle 的 domain 是否一致，不要求 path 完全一致
16. 当客户端跟随 `301` / `308` 迁移到新 Resolution Endpoint 后，必须重新执行双向绑定验证，不得仅依据重定向接受新的绑定关系
17. 对于底层 DID 轮换后的安全敏感操作，客户端必须重新执行双向绑定验证
18. Profile URL 不得作为 Handle→DID 绑定或服务发现的权威来源
...
8. Handle Provider 在返回 429 时应携带 `Retry-After`
9. Handle Provider 应尽量统一错误响应结构，减少不必要的状态侧漏
10. DID 持有者在 Handle Provider 迁移后应更新 DID Document 中的 `HandleService`
11. 当底层路径型 DID 轮换时，Handle Provider 应尽快将 Handle 映射更新到新的 DID
