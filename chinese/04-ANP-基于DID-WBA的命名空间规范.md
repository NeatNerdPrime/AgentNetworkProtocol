...
其中，`HandleService.serviceEndpoint` 表示该 DID 主体认可的、用于解析和验证其 Handle 绑定关系的权威 Handle Resolution Endpoint。它不是 Profile 页面入口，也不是消息服务端点，而是名称绑定校验端点。

`HandleService.serviceEndpoint` 可以指向与 DID 主机不同的域名。其可信性不依赖 Handle 域名与 DID 域名是否一致，而依赖以下两个条件：

1. Handle Provider 正向声明 `Handle -> DID` 映射；
2. DID 持有者在 DID Document 中通过 `HandleService` 反向声明对应的 Handle Resolution Endpoint。

只有两个方向的声明一致时，Handle 绑定关系才应视为可信。
...
若两个方向的声明一致，则绑定关系可信；否则必须（MUST）视为不可信。实现者可以（MAY）进一步向用户告警。

### 6.3.1 HandleService 的使用规则

验证者在执行双向绑定校验时，应按以下规则使用 `HandleService.serviceEndpoint`：

1. 先根据输入 Handle 构造其规范化 Handle Resolution Endpoint；
2. 通过该端点获得 Handle Resolution Document，并提取其中的 `did`；
3. 解析该 `did` 对应的 DID Document；
4. 在 DID Document 的 `service` 中查找 `type = "HandleService"` 的条目；
5. 比较该条目的 `serviceEndpoint` 是否与步骤 1 中的规范化 Handle Resolution Endpoint 一致；
6. 若一致，则说明 DID 持有者接受该 Handle Resolution Endpoint 所声明的绑定关系；
7. 若不存在 `HandleService`，或 `serviceEndpoint` 不一致，则该 Handle 绑定不得视为已验证绑定。
...
2. DID 持有者在 DID Document 中通过 `HandleService` 反向声明 DID → Handle Resolution Endpoint；
3. 验证者同时检查两个方向的一致性。
...
15. 当客户端跟随 `301` / `308` 迁移到新 Resolution Endpoint 后，必须重新执行双向绑定验证，不得仅依据重定向接受新的绑定关系
16. 对于底层 DID 轮换后的安全敏感操作，客户端必须重新执行双向绑定验证
17. Profile URL 不得作为 Handle→DID 绑定或服务发现的权威来源
