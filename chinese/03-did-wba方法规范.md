...
在本规范中，路径型 did:wba 的默认方案会在 DID 路径中携带绑定公钥指纹，用于增强 DID 与用户自持私钥之间的绑定关系。同时，我们设计了一个基于 did:wba 方法和 HTTP 协议的流程：请求签名基于 [RFC 9421](https://www.rfc-editor.org/rfc/rfc9421) 的 HTTP Message Signatures，消息体完整性基于 [RFC 9530](https://www.rfc-editor.org/rfc/rfc9530) 的 `Content-Digest` 字段，使服务端可以在不增加交互次数的情况下快速验证其他平台客户端的身份。
...
wba-did = "did:wba:" domain-name 1*(":" path-segment)
```

#### 2.2.1 默认路径方案：`k1_` 绑定公钥指纹

对于新创建的路径型 did:wba，最后一个 path segment 必须（MUST）是绑定公钥指纹段。当前版本只定义一种默认指纹前缀：

- `k1_`：表示绑定密钥为 secp256k1 公钥，指纹值按本规范 2.2.2 节计算

推荐结构如下：

```plaintext
did:wba:{domain}:{namespace...}:{k1-fingerprint}
```
...
```plaintext
did:wba:example.com
did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
did:wba:example.com%3A3000:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
```
...
`k1_` 指纹用于将路径型 DID 与用户实际控制的绑定公钥关联起来。其生成方法如下：
...
6. 在编码结果前面添加 `k1_` 前缀，得到最终路径段。最终字符串总长度通常为 46 个字符。
...
```json
{"crv":"secp256k1","kty":"EC","x":"NtngWpJUr-rlNNbs0u-Aa8e16OwSJu6UiFf0Rdo1oJ4","y":"qN1jKupJlFsPFc1UkWinqljv4YE0mq_Ickwnjgasvmo"}
```

SHA-256 后经 base64url（无 padding）编码得到：

```plaintext
WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
```

最终路径段为：

```plaintext
k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
```

> 说明：
>
> 1. `k1_` 前缀不是哈希输出的一部分，而是算法/类型前缀；
> 2. 推荐将不带 `k1_` 前缀的原始 thumbprint 值用作该验证方法的 `kid`，以便 DID 路径段与验证方法标识之间保持直观对应；
> 3. 后续版本可以扩展新的前缀，以支持不同类型的绑定密钥或不同的指纹生成方法。
> 4. 任何路径绑定指纹前缀都必须唯一对应一种确定的指纹生成 profile。
不同 profile 不得共享同一前缀。
...
```json
{
  ...
  "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q",
  "verificationMethod": [
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q",
      "type": "EcdsaSecp256k1VerificationKey2019",
      "controller": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q",
      "publicKeyJwk": {
        "crv": "secp256k1",
        "x": "NtngWpJUr-rlNNbs0u-Aa8e16OwSJu6UiFf0Rdo1oJ4",
        "y": "qN1jKupJlFsPFc1UkWinqljv4YE0mq_Ickwnjgasvmo",
    ...
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#keys-p256-1",
    ...
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#key-x25519-1",
  ...
  "authentication": [
    "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q",
    "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#keys-p256-1",
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#key-1",
  ...
  "keyAgreement": [
    "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#key-x25519-1"
  ],
  "service": [
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#ad",
      "type": "AgentDescription",
      "serviceEndpoint": "https://agent-network-protocol.com/agents/example/ad.json"
    },
    {
      "id": "did:wba:example.com%3A8800:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#handle",
      "type": "HandleService",
...
- **id**: 必须字段，不可以携带IP，但是可以携带端口，携带端口时，冒号需要编码为`%3A`。后面使用冒号进行路径分割。对于新创建的路径型 DID，最后一个 path segment 必须（MUST）是绑定密钥指纹段，例如 `k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q`。
...
- **verificationMethod**: 必须字段，包含验证方法的数组，定义了用于验证DID主体的公钥信息。对于需要支持端到端加密（E2EE）通信的场景，`verificationMethod` 中**应**同时包含签名密钥和密钥协商密钥，实现密钥分离。签名密钥（如 EcdsaSecp256r1VerificationKey2019）用于身份认证和 E2EE 签名；密钥协商密钥（如 X25519KeyAgreementKey2019）用于 HPKE 密钥封装。两类密钥各司其职，单一密钥泄露不会同时影响身份认证和通信机密性。
  ...
  对于采用默认路径方案的路径型 DID，`verificationMethod` 中必须（MUST）至少存在一个 secp256k1 的 `publicKeyJwk` 作为**绑定密钥**，其 RFC 7638 thumbprint 与 DID 路径最后的 `k1_` 指纹段完全一致。
...
- **authentication**: 必须字段，列出用于身份验证的验证方法，可以是字符串或对象。对于采用默认路径方案的路径型 DID，绑定密钥必须（MUST）被 `authentication` 授权。默认情况下，跨平台身份认证应优先使用该绑定密钥进行签名。
...
- **proof**：可选字段，用于表达 DID Document 的完整性证明。
  该字段用于证明 DID Document 在生成 proof 之后未被篡改，并表明 proof 创建时签名者控制了对应私钥。
  proof 本身不单独替代 DID method 解析过程，也不单独替代 `id` 一致性检查。
...
>    - 其 RFC 7638 thumbprint 与 DID 路径最后一个 `k1_` 指纹段完全一致。  
>    推荐将不带 `k1_` 前缀的 thumbprint 值作为该验证方法的 `kid`。
...
3. 如果创建的是路径型 DID，先生成 DID 绑定密钥，并按照 2.2.2 节计算 `k1_` 指纹段；
...
如果指定了可选路径而不是裸域名，且采用默认路径方案，则 `did.json` 将在带有 `k1_` 指纹段的路径下可用：

```plaintext
示例5：使用默认路径方案创建路径型 DID
did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
 -> https://example.com/user/alice/k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q/did.json
```
...
示例6：使用可选路径和端口创建DID
did:wba:example.com%3A3000:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q
 -> https://example.com:3000/user/alice/k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q/did.json
```
...
did:wba DID Document 可以（MAY）包含顶层 `proof` 字段，用于提供文档完整性证明。

当 DID Document 包含 `proof` 时，`proof` 对象包含以下字段：

- `type`：必须字段。固定为 `DataIntegrityProof`
- `cryptosuite`：必须字段。当前版本固定为 `didwba-jcs-ecdsa-secp256k1-2025`
- `created`：必须字段。proof 创建时间，采用 XML Schema datetime 格式
- `verificationMethod`：必须字段。完整 DID URL，指向 DID Document 中用于生成 proof 的验证方法
...
1. 复制待签名 DID Document，并移除顶层 `proof` 字段；
...
5. 将 `hash(proofOptions) || hash(document)` 进行拼接；
6. 使用 `verificationMethod` 指向的 secp256k1 私钥对拼接结果签名；
7. 将 ECDSA DER 签名转换为固定长度 64 字节的 `R || S`；
...
2. 检查以下字段是否存在：
   ...
   - `proofPurpose`
   - `proofValue`
3. 检查 `type`、`cryptosuite`、`proofPurpose` 是否符合本规范；
4. 从 DID Document 中移除 `proof`，得到待验签文档；
5. 从 `proof` 中移除 `proofValue`，得到 proof options；
6. 对文档和 proof options 分别做 JCS 规范化；
7. 分别计算 SHA-256 哈希，并拼接为 `hash(proofOptions) || hash(document)`；
8. 解析 `verificationMethod` 指向的公钥；
9. 将 `proofValue` 做 base64url 解码，恢复 64 字节 `R || S`；
10. 使用对应 secp256k1 公钥验签。

`proof.verificationMethod` 指向的验证方法，必须（MUST）与实际用于生成 `proofValue` 的私钥一一对应。

解析 did:wba DID Document 时，proof 校验默认是可选的增强完整性检查。
...
- `keyid`：必须（MUST）为完整 DID URL，指向 DID 文档中的一个验证方法，例如：  
  `did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q`
...
默认情况下，客户端应当（SHOULD）使用 DID 路径最后 `k1_` 指纹段对应的**绑定密钥**进行签名。服务端如果允许其他 `authentication` 验证方法，属于本地授权策略，不改变 DID 的绑定语义。
...
Signature-Input: sig1=("@method" "@target-uri" "@authority" "content-digest");created=1733402096;expires=1733402156;nonce="abc123";keyid="did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q"
...
2. 选择用于签名的验证方法。默认情况下，应优先使用路径型 DID 的绑定密钥。
   - 如果使用 `EcdsaSecp256k1VerificationKey2019`，应使用 secp256k1 曲线上的 ECDSA + SHA-256 进行签名，签名输出采用固定长度 `r || s` 字节串编码；
   - 如果使用 `Ed25519VerificationKey2020`，应使用 Ed25519 算法；
   - 其他算法由对应验证方法类型定义。
...
5. **验证 DID 绑定关系**：
   ...
   - 对于采用默认路径方案的路径型 DID，必须（MUST）验证 DID 文档中存在绑定密钥，且该绑定密钥的 thumbprint 与 DID 路径最后的 `k1_` 指纹段完全一致。
...
4. 根据验证方法类型选择验证算法：
   - 对于 secp256k1 ECDSA，验证值应按固定长度 `r || s` 字节串处理；
   - 对于 Ed25519，按 Ed25519 的 64 字节签名格式处理；
   - 其他算法按对应验证方法类型定义。
...
```json
{
  "sub": "did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q",
...
```json
{
  "auth": {
    ...
    "signatureInput": "sig1=(\"@method\" \"@target-uri\" \"@authority\" \"content-digest\");created=1733402096;expires=1733402156;nonce=\"abc123\";keyid=\"did:wba:example.com:user:alice:k1_WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q#WjKgJV7VRw3hmgU6--4v15c0Aewbcvat1BsRFTIqa5Q\"",
    "signature": "sig1=:BASE64_SIGNATURE:"
  },
  "payload": {
    "orderId": "12345",
...
本版进一步将路径型 DID 的默认方案定义为“在 DID 路径中携带绑定公钥指纹”，以强化 DID 与用户自持私钥之间的绑定关系；同时将跨平台 HTTP 身份认证流程切换为基于 HTTP Message Signatures 与 `Content-Digest` 的标准化方案，并将认证成功后的附加认证信息放入 `Authentication-Info` 响应头中。
