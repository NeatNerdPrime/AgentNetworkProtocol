...
- 如果本地策略启用了 DID Document proof 校验，则：
  - 对 `e1_` DID，上述 proof 校验已经是解析成功的必要条件；
  - 对其他 profile，若文档包含 `proof`，则应按对应 profile 规则进行校验。
...
对于默认 `e1_` profile，DID Document 顶层 `proof` 是必须字段。当 DID Document 包含 `proof` 时，主规范定义的 `proof` profile 必须（MUST）符合：
