...
- 状态：v1.1 已发布基线；Messaging 1.2 多设备草案
- 版本：1.1 已发布 / 1.2 草案
- 语言：中文
- 适用范围：本文档同时索引已发布的 ANP Messaging 1.1 Profile 与混合版本的 ANP Messaging 1.2 多设备草案。
...
ANP 的业务身份和授权主体仍是 **Agent DID**。在已发布的 v1.1 Profile 中，设备和内部副本仍不属于互通边界。Messaging 1.2 草案使普通非 E2EE 私聊、群聊、Mention 和附件操作保持其 v1 DID 定址 contract，端点 fan-out 留在接收域内部。只有 E2EE v2 Profile 公开安全跨域密码学通信必需的最小设备语义：`device_id` 表示同一 DID 下的一个密码学端点，它不是新的业务身份、群成员或 `target.kind`。
...
在 Messaging 1.2 中，宣告设备定址 E2EE v2 Profile 的 DID 还可以在其根签名 DID Document 中携带完整 `deviceManifest`。条目只公开当前设备端点标识、签名 key 引用、E2EE key 引用及支持的 Profile 标识与依赖；仅 Base 发现不要求 Manifest。产品域内角色、token、恢复状态、Registry 版本和文档 checkpoint 都不是 ANP wire 语义。
...
Messaging 1.2 不创建设备级业务成员。P3 Direct Base 投递，以及 P4 的成员、角色、策略、发送和通知都仍在 v1 Profile 下按 DID 管理。只有 P5/P6 v2 E2EE 投递和 MLS 密码学 Leaf 按设备管理。
...
v1.1 基线仍把设备、终端和副本视为 Agent 内部细节。Messaging 1.2 保持普通 Base wire 边界和 v1 Profile 标识不变，只由 P5/P6 v2 调整 E2EE 密码学端点边界：公开避免共享设备 key、共享 Ratchet、隐藏加密投递 fan-out 和 MLS Leaf 歧义所需的最小设备标识及 key 绑定。DID 仍是业务身份，产品域内设备管理仍不属于协议范围。
...
[ANP Messaging 1.2 中文草案集](message/vnext/README.md)保持各 Profile 的独立生命周期。P1/P2/P3/P4/P7/P8 与 P9 Mention binding 保持 v1；只有不兼容的多设备 P5/P6 Profile 使用 v2：

| Profile | Messaging 1.2 草案 | 主要多设备变化 |
| --- | --- | --- |
| P1 | [`anp.core.binding.v1`](message/vnext/01-核心绑定.md) | DID 级通用元数据，以及由 P5/P6 v2 拥有的注册型条件式设备字段 |
| P2 | [`anp.identity.discovery.v1`](message/vnext/02-身份与发现.md) | 面向设备定址安全 Profile 的根签名 `deviceManifest` 和当前资格；Base 发现保持 DID 级 |
| P3 | [`anp.direct.base.v1`](message/vnext/03-私聊基础语义.md) | 一次 DID-to-DID 普通投递，不携带设备 selector |
| P4 | [`anp.group.base.v1`](message/vnext/04-群组基础语义.md) | DID 级成员、发送和通知，端点 fan-out 由所属域在本地完成 |
...
| P7 | [`anp.attachment.v1`](message/vnext/07-附件与对象传输.md) | DID 定址的 manifest、对象控制和 Ticket 流程；E2EE object key 投递继承 P5/P6 |
| P8 | [`anp.federation.relay.v1`](message/vnext/08-联邦与跨域.md) | 针对外层 E2EE v2 Profile 的条件式设备 selector 保留和资格校验 |
| P9 | [v1 binding](message/vnext/09-消息Mention扩展.md) | 不变的 mention payload 与 P4 v1 或 P6 v2 组合；mention 仍使用 DID/群 selector |
...
- **联邦制**：像 Email 一样跨域互通；
- **身份优先**：以 DID 为统一锚点；
- **设备安全的 E2EE v2**：一个 DID 可以公开多个独立密码学端点，而不改变业务身份或 Base v1 contract；
