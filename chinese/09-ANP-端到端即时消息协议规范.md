...
| `e2ee_ack` | 私聊 E2EE | 会话确认（接收方确认已成功接受会话） |
...
**E2EE 约束：**
- 私聊 E2EE 消息（type 为 `e2ee_init`、`e2ee_ack`、`e2ee_msg`、`e2ee_rekey`、`e2ee_error`）必须指定 `receiver_did`，不能指定 `group_did`
...
- `server_seq` 由**本地 Message Server** 在消息成功持久化时分配，从 1 开始单调递增。
- 适用范围：
  - **单服务端部署**：实现方可以在“私聊 DID 对”或 `group_did` 维度内维护递增序号。
  - **跨平台 / 跨 Message Server 互联**：`server_seq` 仅表示**当前服务端本地视角**下的排序号，不要求在不同服务端之间全局可比。
- `server_seq` 保证单调递增，但**允许存在 gap**（例如因数据库分配策略导致的编号间隔），**不允许回退**。
- 所有查询接口返回的消息列表**必须**按 `server_seq` 升序排列。

**客户端 gap 检测与补拉：**

- 客户端**应**记录每个“本地服务端数据流”的已接收最大 `server_seq`。
...
| **维度** | 单个 Message Server 的本地消息流（私聊或群聊视图） | 私聊=session_id，群聊=(sender_did, epoch, sender_key_id) |
...
本协议按**密钥角色**严格分离身份认证、proof 签名和密钥协商能力。具体 key fragment 名称不是协议常量，优先由 DID 文档声明，AD 文档可作为可选偏好提示：

| 用途 | 算法 | DID 文档字段 | 说明 |
|------|------|-------------|------|
| 身份认证（identity auth key） | 遵循 DID WBA 规范 | `authentication` | 用于 DID WBA 身份认证 |
| E2EE proof 签名（e2ee proof key） | ECDSA secp256r1 | `assertionMethod`（或等价声明） | 用于 `proof.verification_method` 指向的 E2EE proof 签名 |
| 密钥协商（e2ee agreement key） | X25519 | `keyAgreement` | 用于 HPKE KEM 封装/解封装 |

身份认证密钥、E2EE proof 签名密钥和密钥协商密钥不应混用。当前参考实现可默认使用 `key-1`、`key-2`、`key-3` 作为本地约定，但协议本身不写死这些 fragment 名称。
...
4. Alice 对整个 content 做 proof 签名（使用 DID 文档中声明的 E2EE proof 签名密钥）。
5. Alice 通过 `send` 方法发送 `type=e2ee_init` 消息。Alice 发送后**可以**直接发送 `e2ee_msg` 加密消息，但这只代表发起方本地已具备发送所需状态，**不代表接收方已经成功接受该会话**。
...
9. Bob **应**发送 `e2ee_ack` 确认自己已成功接受该会话。Alice 收到 `e2ee_ack` 后，可将该会话视为“远端已确认”。

**消息送达顺序要求：**

Message Server **应**（SHOULD）尽力保证同一发送方到同一接收方的消息按发送顺序送达（FIFO 顺序），但在分布式系统中绝对的 FIFO 送达不总是可行。协议通过 `server_seq` 提供**可检测、可恢复**的顺序保证：

- 接收方通过**本地服务端返回的** `server_seq` 检测同一数据流内部是否存在 gap。若出现 gap，接收方**应**通过 `get_history(since_seq=...)` 补拉缺失消息。
- `e2ee_init` 消息**应**在后续 `e2ee_msg` 消息之前送达接收方。若接收方先拿到 `e2ee_msg` 而找不到对应 `session_id` 的会话状态，**不得**立刻把该消息视为永久失败；实现方**应**保留该消息的未处理状态，并在补拉缺失消息或后续再次处理 inbox 时重试。
- `e2ee_rekey` 与后续 `e2ee_msg` 同理，使用新 `session_id` 的密文在对应 `e2ee_rekey` 被成功处理前，不应被永久丢弃。
- 对于群聊，`group_e2ee_key` **应**在同一发送方的 `group_e2ee_msg` 之前送达给对应接收方。缺 key 时同样应保留未处理状态并在依赖满足后重试。
...
| `decryption_failed` | 解密失败（密钥不同步等） | 发送方可选择重发，或在必要时先 `e2ee_rekey` 再重发 |
| `invalid_seq` | 序号不匹配 | 发送方发送 `e2ee_rekey`，并自行决定是否重发失败消息 |
| `proof_expired` | 控制消息 proof 过期 | 发送方可选择重新发送控制消息 |
| `proof_from_future` | 控制消息 proof 时间超前过多 | 发送方检查本地时钟后决定是否重发 |
| `unsupported_version` | 对方使用了不支持的 E2EE content 版本或未携带版本号 | 升级对端到 `e2ee_version = "1.1"` 后再重试 |
...
所有 E2EE content（私聊 + 群聊，控制消息 + 密文消息）**必须**包含 `e2ee_version` 字段。当前版本固定为字符串 `"1.1"`。若接收方收到缺少 `e2ee_version` 或版本值不支持的消息，**不需要兼容旧格式**，而应返回 `e2ee_error`（`error_code = "unsupported_version"`）告知对方升级。

#### 8.4.1 e2ee_init — 会话初始化

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| e2ee_version | string | 是 | E2EE content 版本号，固定为 `"1.1"` |
...
| expires | integer | 否 | 控制消息可被接受并建立/更新状态的最大寿命（秒），默认 86400 |
...
| verification_method | string | 签名使用的验证方法 ID（DID 文档中声明的 E2EE proof 签名方法） |
...
|------|------|------|------|
| e2ee_version | string | 是 | E2EE content 版本号，固定为 `"1.1"` |
| session_id | string | 是 | 被确认的会话 ID |
| sender_did | string | 是 | 发送确认的一方 DID |
| recipient_did | string | 是 | 确认接收方 DID |
| expires | integer | 否 | ACK 控制消息可被接受的最大寿命（秒），默认 86400 |
| proof | object | 是 | 发送方签名 |

`e2ee_ack` 用于告诉会话发起方：接收方已经成功处理了对应的 `e2ee_init` 或 `e2ee_rekey`。`e2ee_ack` 不是建立会话所必需的密码学步骤，但它提供了一个明确的远端确认信号。
...
| failed_msg_id | string | 否 | 若错误针对某条具体密文消息，返回该消息 ID |
| failed_server_seq | integer | 否 | 若接收方能识别失败消息在本地服务端的排序号，返回该 `server_seq` |
| retry_hint | string | 否 | 接收方建议动作：`resend` / `rekey_then_resend` / `drop` |
| required_e2ee_version | string | 否 | 若错误为版本不兼容，返回接收方要求的 `e2ee_version` |
...
需要签名的 E2EE 消息包括：`e2ee_init`、`e2ee_ack`、`e2ee_rekey`、`group_e2ee_key`、`group_epoch_advance`。
...
5. **验证时间戳**：控制消息的 freshness 校验必须区分“未来偏移”和“过去年龄”。实现方**应**限制 `proof.created` 不得超前本地时间过多（推荐上限 300 秒），并检查消息年龄 `now - created` 不得超过 `min(expires, 本地策略上限)`；对离线消息，**不应**仅使用对称的 ±300 秒窗口判断是否过期。
...
- **proof 时间戳**：签名中的 `created` 时间戳用于控制消息 freshness 校验；实现方应同时校验未来偏移和过去年龄，而不是只做对称时钟漂移检查。
