...
- `capabilityInvocation` 属于可选扩展能力，**MAY** 存在，但不属于 v2 最小互通要求。
...
服务端点对象 **MAY** 携带少量静态 hint，但 v2 标准互通只要求：
...
为了降低跨域服务选择歧义，v2 只鼓励对外暴露一个统一的 `ANPMessageService`。下图把这个统一入口与常见内部逻辑角色之间的关系放在一起，帮助读者区分“公开服务类型”和“实现内部分工”。
...
本节仅用于**非规范性说明**统一入口背后的常见能力边界；它们不是 DID 文档中的独立标准 service type，也不是 v2 的服务选择字段。
...
为了便于跨实现发现，本 Profile 只把少量字段作为 v2 静态 hint；其余能力信息 **SHOULD** 下沉到 `anp.get_capabilities`。
...
- 要求：**不作为 v2 标准 DID hint**
- 说明：
  - v2 规范不依赖 DID 文档中的 `priority` 做服务选择；
  - 若部署自定义了该字段，应视为私有扩展。
...
v2 规范 **不**依赖 DID 文档中的 `supportedMethods`、`logicalRoles` 或 `priority` 做标准服务选择。
...
- 若对象是治理能力调用行为，验证方 **MAY** 检查 `capabilityInvocation`，但这不是 v2 最小互通要求。
