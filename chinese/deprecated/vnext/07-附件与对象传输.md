...
- **Object URI**：对象定位 URI。v2 中它是一个 locator 风格的 HTTPS 资源地址，不等价于公开直链。
...
- **Nonce**：与对象加密一起使用的随机数；在 v2 MTI 中长度固定为 12 字节。
...
v2 中，`encryption_info.mode` **MUST** 只允许以下两个取值：
...
为保持 v2 清晰、简单、可落地，以下路径 **不**进入本 Profile：
...
当附件清单通过 `direct.send` 在 `anp.direct.base.v2` 中发送时：
...
当附件清单通过 `anp.direct.e2ee.v2` 发送时：
...
当附件清单通过 `anp.group.e2ee.v2` 下的 `group.e2ee.send` 发送时：
...
- P5 Direct E2EE 外层携带其要求的发送和接收设备字段；P6 MLS 外层携带其要求的设备绑定。这些外层字段只由 P5 或 P6 解释。
...
- `digest.alg` 在 v2 **MUST** 固定为 `sha-256`
...
2. `object_uri` 在 v2 中表示 locator 风格的对象地址；它不是永久公开直链
...
v2 不定义专门的 `thumbnail` 字段，也不定义专门的 `chunking_info` 字段。
...
v2 的 `object-e2ee` MTI 算法 **MUST** 为：
...
在 v2 MTI 的 `chacha20-poly1305` 下：
...
- v2 **不需要** 再为附件单独设计复杂的密钥协商协议
- 在群场景下，能解密该群消息的人，就都能拿到这把对象密钥
- v2 **不保证** 追溯性撤权；被移出群的成员若此前已拿到对象密钥和对象内容，本规范不承诺事后抹除其已获得的信息
...
v2 **不单独标准化** Access Grant 在多个内部服务之间的同步协议。

在 v2 默认路径中，下载票据由原始附件消息发送者公开的 `ANPMessageService` 负责签发；该公开服务入口在内部是否再路由到独立 Object Service，属于实现细节。若部署方在内部拆分了对象控制组件，**MUST** 保证对外公开服务能够以本地状态或内部同步方式拿到签发下载票据所需的 Access Grant。
...
- v2 **不要求** 在 Object 控制服务端再次直接验证独立终端的 `origin_proof`
...
18. 支持 `mode = "object-e2ee"`
19. 支持本 Profile 规定的 `chacha20-poly1305` 对象加密流程
20. 在受 P5 或 P6 保护的 E2EE 附件 manifest 内分发 `object_key_b64u` 与 `nonce_b64u`；设备选择仍由对应 overlay 负责
21. 在对象下载后执行密文字节摘要校验与本地解密
22. 明确遵守“v2 不保证追溯性撤权”的访问边界
...
5. 若确实需要更复杂的对象能力，应在未来新版本单独定义，而不是把 v2 主线路径重新分叉
