...
- 依赖：`anp.core.binding.v2`、`anp.identity.discovery.v2`、`anp.group.base.v2`
...
- **MLS Controller**：负责执行 MLS 成员变更控制动作的主体。v2 中固定为群 `owner`。
...
- **MLS Member Credential Rebind**：在 P4 已接受同一 Handle 成员的 DID 换绑后，通过有序 Commit 加入选定的新 DID 设备 Leaf，再移除全部旧 DID 设备 Leaf 的密码学编排。
...
P4 的成员、角色、策略与成员计数仍归属当前 `agent_did`。P4 的可选 `member_handle` 是业务成员稳定锚点，但 **MUST NOT** 替代 MLS `credential.identity` 中的当前 DID。
...
- 某设备丧失 P2 Manifest 资格，但其 DID 仍为 `active` → owner 只移除该设备 Leaf
- 某 Handle-backed Member 在 P4 中产生 `member-credential-rebound` → owner 加入选定的新 DID 设备 Leaf，再移除所有旧 DID 设备 Leaf

### 3.5 owner 是唯一 MLS 控制者

v2 中，**只有 owner DID 承担 MLS 控制者角色**。控制请求由一台合格且持有所需当前 MLS 状态的 owner 设备提交；另一台 owner 设备具备资格，不等于它持有该私有状态。
...
- 执行 `add`；
- 执行 `remove`；
- 通过有序的逐设备 Add 与 Remove 执行成员凭证换绑；
- 生成成员变更对应的 `commit`；
- 为新成员生成 `welcome`；
- 推进成员变更后的 `epoch`。
...
OBS -->|Handle 成员已换绑且旧 Leaf 仍存在| REBIND_ADD
...
Handle-backed Member 的凭证换绑不引入新的 P6 方法或 MLS 原语；它先为每个选定的新 DID 设备产生一次 `group.e2ee.add` Commit，再为每个旧 DID 设备 leaf 产生一次 `group.e2ee.remove` Commit。
...
- 对凭证换绑编排产生的 notice，`group_state_ref` **MUST** 精确引用每个 P6 Add 和 Remove 共同使用的已接受 P4 `member-credential-rebound` 事件。接收方 **MUST** 从该 P4 事件取得 Handle、绑定代次、旧 DID 与新 DID，不得依赖 P6 notice 重复携带连续性字段；
...
- 实际接收通知的设备只由外层 `meta.target.did` 与 `meta.recipient_device_id` 标识；在 `commit-delivery` 中，它可以与受影响 subject 不同；
...
- 换绑路径中，`group_state_ref` **MUST** 精确引用已接受的 P4 `member-credential-rebound` 事件，`member_did` **MUST** 等于该事件的 `subject_did`，且新 KeyPackage **MUST** 绑定选定的新 DID 设备；
- 选定新 DID 设备的所有换绑 Add **MUST** 在移除旧 DID 设备 Leaf 之前成功。
...
由 owner 执行 MLS `remove`，每次精确移除一个设备 Leaf。触发条件可以是 P4 移除该 DID、在 DID 仍为 active 时设备丧失资格、群策略，或已接受的 Handle 换绑编排。
...
- 换绑路径中，`member_did` **MUST** 等于所引用 P4 事件的 `previous_subject_did`，选定新 DID 设备的 Add 必须已成功，且 owner 必须在恢复应用消息前移除每个旧 DID 设备 Leaf。
...
当只有一台设备丧失当前 Manifest 资格，或在其 DID 仍为 P4 `active` 时被群策略移除，owner **MUST** 只移除该设备 Leaf。这不改变 P4 成员或 sibling Leaf。

Group Host 一旦确定某当前 Leaf 必须移除，就 **MUST** 暂停接受新 `group.e2ee.send`，直到对应 Remove Commit 将群推进到不再包含该 Leaf 的 epoch。只停止未来服务投递不是密码学移除。
...
3. 每个 Remove 都成功后完成密码学换绑。

所有请求 **MUST** 精确引用同一个 P4 `member-credential-rebound` 的 `group_state_ref`。全部选定 Add **MUST** 在第一个 Remove 之前成功；Remove 只能删除该事件 `previous_subject_did` 的设备 Leaf，不得借换绑流程移除其它成员。
...
从 P4 换绑事件被接受起，到最后一个 Remove 成功为止，Group Host **MUST** 暂停接受该群新的 `group.e2ee.send`。Add 与 Remove 产生的中间 epoch **MUST NOT** 承载应用消息。某一步失败时，实现 **MUST** 保持暂停并重试该步骤。

P6 失败 **MUST NOT** 回滚 P4 换绑、恢复旧 DID 的 P4 权限，或创建新的 P6 业务成员状态。P4 成员资格、角色、状态、加入时间和成员数量始终由原换绑事件决定。

若换绑对象本身是 owner，一台被选中的新 DID 合格设备 **MUST** 使用自己的设备绑定 `origin_proof`。过渡 Add Commit **MAY** 使用一个旧 owner 设备 Leaf 保留的 MLS 状态生成。至少一个新 owner 设备 Leaf 加入后，后续旧 DID Remove **MUST** 由一台当前合格且持有当前状态的新 owner 设备 Leaf 生成。若没有已授权 owner 设备保留或获得所需当前状态，系统 **MUST** fail closed，并保持 E2EE 消息面暂停。
...
3. 校验目标 DID 是普通路径中的 P4 `active` 成员，或是所引用 P4 `member-credential-rebound` 事件中的 `subject_did`；
...
在凭证换绑编排中，本节的每个 Add **MUST** 引用 P4 换绑事件的 `group_state_ref`，加入一个绑定新 DID 的选定设备 leaf，并且只向该 DID/设备组合交付 Welcome。所有选定的新 DID 设备 Add 都先于任何旧 DID 设备 leaf 的移除。
...
2. 校验存在一种允许的触发条件：该 DID 在 P4 中为 `removed` 或 `left`、该设备不再满足当前 P2 Manifest 或群策略资格，或该组合是所引用 P4 换绑事件中的旧 DID leaf；
...
在凭证换绑编排中，只有全部选定的新 DID 设备 Add 都使用同一 `group_state_ref` 成功后才能开始 Remove；每个 Remove 只能删除该 P4 事件标识的一个旧 DID 设备 leaf。
...
4. 使用 Welcome 和 ratchet tree 初始化并持久化该设备自己的 MLS group state；
5. 把该状态绑定到本地 `(group_did, device_id)` 组合，并准备接收后续 commit 和应用消息。
...
- 当前业务层已同步的成员集视图
- 最近一次成功应用的 `add/remove` 结果引用，包括凭证换绑编排中的每个操作
...
1. 验证对应 P4 `member-credential-rebound` 状态事件及其中的新旧 DID 和 `group_state_ref`；
...
6. 将每个 Commit 独立投递给每个保留设备 leaf；
7. 仅在最后一个 Remove 成功后恢复应用消息。

每个 Add 和 Remove Commit **MUST** 绑定同一个 P4 `group_state_ref`。每个中间 epoch 只用于完成换绑，**MUST NOT** 承载应用消息。新 DID 设备只通过自己的 Welcome 获得当前及后续状态；本 Profile **MUST NOT** 恢复已丢失的历史 epoch secret，也不得在设备之间共享私有 MLS 状态。

若换绑目标是 owner，过渡 Add Commit **MAY** 由保留当前状态的合格旧 owner 设备生成。某个新 owner 设备 leaf 加入后，后续旧 DID Remove **MUST** 由具有当前状态的合格新 owner 设备生成。若没有已授权 owner 设备保留或获得当前状态，实现 **MUST** fail closed；本 Profile 不提供自动 owner MLS 恢复，Group Host 也 **MUST NOT** 下发当前或历史 epoch secret。
...
Group Host **MUST** 为每个保留设备 leaf 生成一个独立通知信封。每个信封以该 leaf 的 DID 和设备 ID 为目标，而 `body.subject_did` 和 `body.subject_device_id` 标识该 Commit 添加或移除的一个 leaf。设备收到后按照其本地 MLS runtime 规则处理该 commit。
...
- 对凭证换绑编排中的 Add，目标 DID **MUST** 等于事件 `subject_did`；不得向 `previous_subject_did` 或未被选中的新 DID 设备发送 Welcome。
...
- `params.auth` 无损保留原始 `scheme` 和 `origin_proof`；
...
对于凭证换绑编排，每个 Commit **MUST** 使用上述绑定并引用同一个 P4 `group_state_ref`：每个 Add 的 `member_did` **MUST** 等于事件的 `subject_did`，每个 Remove 的 `member_did` **MUST** 等于事件的 `previous_subject_did`。P6 请求不增加 `member_handle`、`handle_binding_generation`、`previous_member_did` 或 `new_member_did`；这些连续性字段由所引用的 P4 事件提供。
...
1. `auth.origin_proof` 合法
2. `meta.sender_did` 当前是群 `owner`，且 `meta.sender_device_id` 是具备所需状态的合格当前 owner leaf；
3. `group_state_ref.group_did` 与外层目标一致
...
- 普通 Add **MUST** 以当前 P4 `active` DID 的合格 Manifest 设备为目标，且精确 DID/设备组合尚未成为 leaf；换绑 Add **MUST** 精确引用 P4 `member-credential-rebound`，以事件 `subject_did` 为目标，并验证 KeyPackage 绑定该 DID 的被点名设备；
- 普通 Remove **MUST** 以一个现有 leaf 为目标，且该 leaf 的 DID 已在 P4 中为 `removed` 或 `left`，或其设备在 DID 仍为 `active` 时失去 Manifest 资格或被群策略移除；换绑 Remove **MUST** 在全部选定新 DID 设备 Add 成功后，以同一事件和 `group_state_ref` 下的旧 DID 设备 leaf 为目标；
- Group Host **MUST** 拒绝先于对应 Add 的换绑 Remove，也 **MUST** 拒绝借换绑例外移除其它 `active` 成员；
- 换绑期间，Group Host **MUST** 串行化全部选定 Add 与全部旧 leaf Remove，并拒绝或延后无关的 Add/Remove；
...
普通动作中，两个绑定 **MUST** 解析为同一 DID/设备组合，且 **MUST NOT** 互相替代。唯一例外是第 10.5 和 11.12 节的 owner 换绑过渡 Add：Origin Proof 绑定当前合格的新 owner 设备，Commit 可由保留状态的旧 owner 设备 leaf 生成，二者都绑定同一个已接受 P4 换绑事件。
...
- 只有 P4 owner DID 能授权 `group.e2ee.create/add/remove`
...
- P4 移除某 DID 或该 DID 离群时，**MUST** 通过移除该 DID 的每个 leaf 完成收敛；单台设备失去资格时只移除该 leaf，不改变兄弟 leaf 或 P4 成员资格；
- 多设备换绑期间的每个中间 epoch **MUST NOT** 承载应用消息；
...
15. 由 P4 `member-credential-rebound` 驱动全部选定的 `add(new DID, device)`，随后执行全部 `remove(old DID, device)`，且每个步骤绑定同一 `group_state_ref`
16. 从 P4 换绑被接受到最后一个 Remove 完成期间暂停应用消息，并在全过程串行化成员变更控制动作
...
27. P4 移除某 DID 或该 DID 离群时移除其全部 leaf，单台设备失去资格时只移除受影响 leaf
...
P4 `member-credential-rebound` 已被接受且消息面暂停后，owner 为一个选定的新 DID 设备提交如下 Add（其它选定设备重复该操作）：
...
Group Host 向每个当前设备 leaf 独立发送该信封。每份信封携带同一组排序字段、回执和 `group_cipher_object`；只有目标 DID 和 `recipient_device_id` 随 leaf 改变。
...
- 通过 P4 `member-credential-rebound` 驱动先按设备 add(new DID) Commit、再按设备 remove(old DID) Commit 的有序流程；
