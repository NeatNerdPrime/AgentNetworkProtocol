...
- 依赖：`anp.core.binding.v2`、`anp.identity.discovery.v2`、`anp.direct.base.v2`、`anp.group.base.v2`
...
> 说明：`anp.federation.relay.v2` 是本草案的 vNext 标识。
...
对于当前 P4 v2 核心中的 `group.join` 与 `group.add`，跨域成功语义以 Group Host 返回的业务结果为准；在当前 v2 主线下，成功即表示相应业务成员状态已经成立。若部署方额外引入带外凭据、审批流或其它治理中间态，属于扩展路径，不属于本 Profile 的 v2 核心成功语义。
...
这些方法在 v2 最小互通中 **不**假定匿名访问；调用方身份、限流和防滥用控制 **MUST** 由 hop / service 级认证保证。

群 E2EE onboarding 所需的 `welcome` / `ratchet_tree` 等密码学结果由 `group.e2ee.notice` 交付；v2 不定义独立的 `group.e2ee.get_join_info` 标准跨域路由。
...
`group.e2ee.notice` 可以向尚未完成 MLS bootstrap 的目标 Agent 定向交付 `welcome-delivery`，也可以向现有成员交付 `commit-delivery`；这属于 P6 的密码学结果分发，而不是 P4 的群成员广播。带外邀请凭据或其它非成员治理消息若存在，属于部署扩展，不构成 v2 标准跨域路径。
...
对当前 P7 v2 主线下的 `attachment.*` 控制面方法，通常不存在终端业务 `auth.origin_proof`；外层 `serviceDid` hop 认证 **MUST NOT** 被误认为业务原发者证明。
...
v2 标准互通路径 **MUST** 以“域服务代理模式”为主线：

#### 模式 A：域服务代理模式（v2 标准路径）
