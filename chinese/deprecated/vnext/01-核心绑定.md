...
- **Profile**：ANP 的协议能力单元，例如 `anp.direct.base.v2`。
...
B[Binding 层<br/>anp.core.binding.v2<br/>JSON-RPC 2.0 / meta / auth / body]
...
- 要求：**MAY**
- 语义：ANP 主版本提示
- 默认值：若省略，接收方 **MUST** 按 `"2.0"` 解释
- 说明：
  - 该字段保留用于显式调试或跨版本网关；
  - v2 中优先以 `profile` 决定解释规则；
...
`.v1` 与 `.v2` Profile 标识是相互独立的互通契约。实现 **MUST NOT** 为 v1 对端推断 v2 设备端点、合成默认设备，或把失败的 v2 设备定址 E2EE 操作静默降级为 v1 或 transport-protected Base 操作。这不会使 v2 Base 消息变成设备定址；只有所属 P5/P6 类 E2EE Overlay 才能启用 6.2.4 节的条件式 selector。
...
* 对 `did:wba` 的 DID 解析、验证方法授权关系和路径指纹绑定校验，遵循对应 **did:wba 方法规范**
...
- 若部署使用 `did:wba`，则其方法特定解析、路径指纹绑定及其它附加校验，遵循对应的 DID 方法规范，例如 **[did:wba Method Specification](https://agentnetworkprotocol.com/en/specs/03-did-wba-method-specification/)**。
...
本 v2 Profile 中，以下对象 **MUST** 使用本附录：
