...
- 依赖：`anp.core.binding.v2`、`anp.identity.discovery.v2`
- 适用范围：本 Profile 适用于基于 Group DID 的群生命周期、群管理与群消息基础语义，不包含群端到端加密算法本身。

---

> 说明：本版本将 v2 核心收敛为“自助加入、直接加人”两条路径：
>
> 1. `group.invite`、`group.accept_invite` 与标准 `invitation` 对象已移出 v2 核心；
> 2. `membership_request`、`membership_request_digest`、`group.approve_membership`、`group.reject_membership` 已移出 v2 核心；
> 3. `group_policy` 收敛为 `message_security_profile + bootstrap_security_profile + admission_mode + permissions`；
> 4. 非成员治理定向通知 `group.governance_notice` 已移出 v2 核心；
...
6. 可选 Handle 稳定成员锚点、当前 DID 凭证换绑与群身份连续性。
...
- **Member Handle**：符合 WNS 规范的完整 Handle，当它被记录到 `group_member` 时，作为该成员关系的永久稳定锚点。
- **Handle-backed Member**：具有已验证 `member_handle` 的群成员；其 Handle 表示稳定成员关系，`agent_did` 表示当前密码学凭证。
- **Handle Binding Generation**：来自 WNS Handle Resolution Document 的单调 `binding_generation`，在 P4 中使用十进制字符串 `handle_binding_generation` 表示。
- **Member Credential Rebind**：在不改变成员角色、状态和加入时间的前提下，将 Handle-backed Member 的当前 `agent_did` 从旧 DID 原子更新为新 DID。
...
### 3.8 Handle 成员锚点与 DID 凭证分离

本 Profile 允许 Agent 以 DID-only 或 Handle-backed 两种模式成为群成员：

- DID-only 成员的成员关系与当前 `agent_did` 同一；若 DID 文本发生变化，本 Profile 不提供自动连续性。
- Handle-backed Member 的 `member_handle` 是稳定成员锚点，`agent_did` 是当前用于认证、签名、路由和安全 Overlay 的凭证。
- Handle-backed Member 必须（MUST）使用永久不可转让、不可复用的 WNS Handle；Group Host 必须（MUST）把 Handle Provider 视为该 Handle 连续性的信任根。
- Handle 自身被 `revoked` 时不得（MUST NOT）执行成员凭证换绑；Handle 保持 `active` 而底层 DID 变更时，应使用 `group.rebind_member`。

在 ANP 线协议中，Handle-backed Member 的稳定成员唯一键是 `(group_did, member_handle)`，其 `agent_did` 可以换绑；DID-only 成员的唯一键是 `(group_did, agent_did)`。实现可以在客户端和服务端内部把两者映射为本地 User ID，但该 User ID 不进入 ANP。

成员关系建立后，DID-only 与 Handle-backed 模式不得（MUST NOT）被静默互换或仅凭相同 DID 自动合并。希望获得 Handle 连续性的 Agent 必须在创建该成员关系时提交 Handle；本版本不定义事后给 DID-only 成员附加 Handle 的方法。
...
| Handle 成员换绑 DID | `group.rebind_member` | 旧 DID 失效，新 DID 成为当前凭证 | `group_member` 保持 `active` | 换绑被 Group Host 接受时 | 只适用于 Handle-backed Member |
...
stateDiagram-v2
    ...
    Active --> Active: group.rebind_member
```
...
当使用 Handle-backed Member 时，本 Profile 还 **MUST** 依赖 [ANP 基于 DID-WBA 的命名空间规范（WNS）](../../04-ANP-基于DID-WBA的命名空间规范.md) 对 Handle 格式、解析、双向绑定、`binding_generation`、永久不可转让与永久不可复用语义的定义。
...
```mermaid
flowchart TD
COps[群状态变更操作<br/>create / join / add / rebind / remove / leave / update_*]
Msg[group.send]
...
- `agent_did`：字符串，**MUST**
- `member_handle`：WNS 完整 Handle 字符串，Handle-backed Member **MUST**，DID-only 成员 **MUST NOT**
- `handle_binding_generation`：正十进制字符串，Handle-backed Member **MUST**，DID-only 成员 **MUST NOT**
- `role`：字符串，**MUST**
- `status`：字符串，**MUST**
...
- `agent_did` 始终表示该成员的当前 DID，用于认证、授权判断、消息发送、路由和安全 Overlay；
- `member_handle` **MUST** 是按 WNS 规范归一化后的完整 Handle，并 **MUST NOT** 在 `group.rebind_member` 中改变；
- `handle_binding_generation` **MUST** 精确复制建立或最近一次换绑时经验证的 WNS `binding_generation`；
- Group Host **MUST NOT** 仅凭 Handle 文本建立或换绑成员关系，必须先完成 WNS 解析与绑定验证。

本 Profile 不定义也不在线协议中传输 `user_id`、`member_user_id` 或其它本地数据库标识。客户端与服务端可以（MAY）在内部把 `group_member` 投影到本地稳定主键，但该主键不是 ANP 群成员标识，也不得（MUST NOT）作为跨域认证或授权依据。

`group_member` **MUST NOT** 包含设备标识或列表、设备 selector、设备级角色或状态，也不得包含 MLS Leaf 标识。
...
5. 若 `subject_method = "group.rebind_member"`，则 `event_type` **MUST** 为 `member-credential-rebound`。

#### 7.10.2 标准 `event_type`

本 Profile v2 推荐以下 `event_type`：

- `member-activated`
- `member-removed`
- `member-left`
- `member-credential-rebound`
...
- `member-credential-rebound` **MUST** 包含 `subject_handle`、`previous_subject_did`、`subject_did`、`handle_binding_generation` 与 `membership_status = "active"`
...
12. `body.creator_handle` **MAY** 存在

若 `body.creator_handle` 存在，Group Host **MUST** 按 WNS 规范解析并验证该 Handle，且解析结果必须为 `active`、其 `did` **MUST** 精确等于 `meta.sender_did`。成功后，创建者的 `group_member` **MUST** 记录 `member_handle` 与对应 `handle_binding_generation`；若该字段不存在，创建者是 DID-only 成员。
...
- 每个条目 **MUST** 恰好包含 `member_did` 或 `member_handle` 之一，不得同时包含或同时省略；
- Handle 条目 **MUST** 按第 8.4 节的 Handle 路径解析、验证并保存；
...
若请求包含 `creator_handle`，成功响应 **MUST** 包含 `creator_handle` 与 `handle_binding_generation`；否则这两个字段 **MUST NOT** 出现。
...
`group.join` 用于非成员自主发起加入。对于 v2 核心，成功时调用方立即成为 `active` 成员。
...
若 `member_handle` 不存在，调用方按 DID-only 模式加入，成员 DID 为 `meta.sender_did`。若 `member_handle` 存在，Group Host **MUST**：

1. 按 WNS 规范归一化、解析并完成双向绑定验证；
2. 要求解析文档的 `status = "active"`；
3. 要求解析得到的 `did` **MUST** 精确等于 `meta.sender_did`；
4. 在成员记录中保存规范化后的 `member_handle` 和解析得到的 `binding_generation`。
...
- `group_did`
- `member_did`，且 **MUST** 等于 `meta.sender_did`
...
若请求包含 `member_handle`，成功响应 **MUST** 包含 `member_handle` 与 `handle_binding_generation`；DID-only 加入的响应 **MUST NOT** 包含这两个字段。
...
若使用 `member_did`，Group Host **MUST** 建立 DID-only 成员。若使用 `member_handle`，Group Host **MUST** 按 WNS 规范归一化、解析并完成双向绑定验证，要求 Handle 为 `active`，并以解析得到的当前 DID 建立 Handle-backed Member；成员记录 **MUST** 同时保存 `member_handle`、当前 `agent_did` 与 `handle_binding_generation`。
...
若请求使用 `member_handle`，成功响应 **MUST** 包含 `member_handle` 与 `handle_binding_generation`；若请求使用 `member_did`，成功响应 **MUST NOT** 包含这两个字段。
...
### 8.12 `group.rebind_member`

#### 8.12.1 语义

`group.rebind_member` 用于 Handle-backed Member 在底层 DID 发生变更后，将该成员的当前凭证从旧 DID 原子换绑为新 DID。它不创建新成员，不改变成员锚点，也不恢复 DID-only 成员。

WNS 映射变化本身只表示“可检测到新绑定”，不得（MUST NOT）自动改写群成员。即使 Group Host 通过轮询或缓存刷新提前观察到新 DID，也必须等待新 DID 对每个群提交并通过本方法；因此，本方法及其严格递增的 `handle_binding_generation` 是群感知 DID 变化的标准机制。

#### 8.12.2 调用者与请求要求

本方法 **MUST** 由新 DID 发起，并满足：

- `meta.security_profile` **MUST** 等于 `transport-protected`；
- `meta.sender_did` **MUST** 等于 `body.new_member_did`；
- `auth.origin_proof` **MUST** 由新 DID 的有效 `authentication` 验证方法产生；
- `body` **MUST** 包含 `member_handle`、`previous_member_did`、`new_member_did` 与 `handle_binding_generation`。
...
```json
{
  ...
  "previous_member_did": "did:wba:example.com:agents:alice:e1_<old-fingerprint>",
  "new_member_did": "did:wba:example.com:agents:alice:e1_<new-fingerprint>",
  "handle_binding_generation": "9"
}
```

Group Host 在接受请求前 **MUST**：

1. 找到同一群内 `status = "active"`、`member_handle` 精确相同且当前 `agent_did = previous_member_did` 的唯一成员记录；
2. 对 `member_handle` 执行新的 WNS 解析与双向绑定验证，不得仅依赖超过当前 TTL 的缓存结果；
3. 要求解析文档 `status = "active"`，且其 `did` 精确等于 `new_member_did`；
4. 要求解析文档的 `binding_generation` 精确等于 `handle_binding_generation`，并按十进制整数比较严格大于成员记录中的旧 `handle_binding_generation`；
5. 验证新 DID 的 DID Document 与新 DID possession proof；不得把旧 DID 的签名作为连续性的充分证明；
6. 确认 `new_member_did` 尚未对应同一群内另一条 `active` 成员关系；
7. 若 Handle 为 `suspended`、`revoked`、无法完成 WNS 验证，或发生任何并发冲突，则 fail closed。

验证成功后，Group Host **MUST** 在一次原子状态变更中：

- 将该记录的 `agent_did` 更新为 `new_member_did`；
- 将 `handle_binding_generation` 更新为新值；
- 保持 `member_handle`、`role`、`status`、`joined_at`、`added_by` 与活跃成员数量不变；
- 推进 `group_state_version` 与 `group_event_seq`；
- 立即停止授权 `previous_member_did` 代表该成员执行新的群操作；
- 产生 `member-credential-rebound` 状态事件。

历史消息、历史 `group_receipt`、历史事件和其中的 DID 或签名 **MUST NOT** 被重写。
...
- `member_handle`
- `previous_member_did`
- `member_did`，且其值为新 DID
- `handle_binding_generation`
- `membership_status`，且 **MUST** 为 `active`
- `group_state_version`

成功响应 **MAY** 包含 `group_receipt`。
...
### 9.4 Handle-backed Member 的 DID 换绑

```mermaid
sequenceDiagram
    participant N as 新 DID
    participant H as Group Host
    participant W as Handle Provider
    participant M as 其它活跃成员

    N->>H: group.rebind_member(handle, old DID, new DID, generation)
    H->>W: fresh Handle resolution + binding verification
    W-->>H: active, new DID, newer generation
    H-->>N: membership preserved + new group_state_version
    H-->>M: group.state_changed(member-credential-rebound)
```
...
`group.rebind_member` 是恢复控制操作，不是已有 MLS 成员可生成的普通 member-only 操作。它 **MUST** 使用 `transport-protected` 和新 DID 的 `origin_proof` 完成 P4 换绑；若群启用 E2EE，当前 owner 作为 P6 MLS Controller **MUST** 自动编排 `group.e2ee.add(new DID)` 后再执行 `group.e2ee.remove(old DID)`，Group Host **MUST** 按 P6 在此期间暂停应用消息，不得把 P4 proof 当作任一 MLS Commit 的替代品。
...
- Handle 只能作为经 WNS 验证的稳定成员锚点，**MUST NOT** 代替 DID 请求签名；
- Handle Provider **MUST** 承诺 Handle 永久不可转让、不可复用，并以永久 tombstone 保留已撤销 Handle；不满足该承诺的 Handle **MUST NOT** 用于 Handle-backed Member；
- Group Host **MUST** 持久化最后接受的 `handle_binding_generation`，并拒绝相同、较旧或无法按十进制比较的代次；
- 缓存的 Handle 解析结果一旦过期，或与请求中的新代次冲突，Group Host **MUST** 重新解析并 fail closed；
- `group.rebind_member` 是新 DID 可在尚未成为当前 `agent_did` 前发起的受限例外；其授权范围仅限完成第 8.12 节定义的同 Handle 凭证换绑；
- Group Host 接受换绑后，旧 DID 的 token、会话和路由缓存如何失效属于部署机制，但旧 DID **MUST NOT** 再通过 P4 授权检查。
...
| 3011 | `group.invalid_handle_binding` | Handle 格式、状态、解析结果或双向绑定验证无效 |
| 3012 | `group.handle_binding_stale` | Handle 绑定代次未严格递增、发生回滚或缓存结果过旧 |
| 3013 | `group.rebind_not_allowed` | 目标不是可换绑的 Handle-backed active member，或 Handle 已暂停/撤销 |
| 3014 | `group.rebind_conflict` | 新 DID 已被其它活跃成员使用，或换绑发生并发状态冲突 |
...
选择 Handle-backed membership 会向有权读取成员列表的调用方披露完整 `member_handle`。实现 **SHOULD** 明确这一隐私边界；公共或匿名成员列表仍 **SHOULD NOT** 因此默认披露 Handle。
...
9. `group.send`
10. `group.rebind_member`
11. `group_did`
12. `group_state_version`
13. `group_event_seq`
14. DID-only 与 Handle-backed 两种成员模式
15. 角色：`owner`、`admin`、`member`
16. 成员状态：`active`、`left`、`removed`
17. `group.state_changed` 的统一事件对象语义及 `member-credential-rebound` 事件
18. 安全传输运行方式
19. 不含设备 selector 或逐设备结果的 DID 级成员关系、角色、治理、消息与通知
20. P4 业务成员与 P6 设备级 MLS Leaf 之间的分离
...
```json
{
  ...
  "params": {
    "meta": {
      "profile": "anp.group.base.v2",
      "security_profile": "transport-protected",
      "sender_did": "did:wba:b.example:agents:bob:e1_<new-fingerprint>",
      "target": {
        "kind": "group",
        "did": "did:wba:groups.example:team:dev:e1_<fingerprint>"
      },
      "operation_id": "op-30005",
    ...
    "auth": {
      ...
      "origin_proof": {
        "contentDigest": "sha-256=:BASE64_SHA256_OF_SIGNED_REQUEST_OBJECT:",
        "signatureInput": "sig1=(\"@method\" \"@target-uri\" \"content-digest\");created=1774789200;expires=1774789260;nonce=\"n-30005\";keyid=\"did:wba:b.example:agents:bob:e1_<new-fingerprint>#key-1\"",
        "signature": "sig1=:BASE64_SIGNATURE:"
      }
    },
    "body": {
      "member_handle": "bob.b.example",
      "previous_member_did": "did:wba:b.example:agents:bob:e1_<old-fingerprint>",
      "new_member_did": "did:wba:b.example:agents:bob:e1_<new-fingerprint>",
      "handle_binding_generation": "4"
    }
  }
}
```
...
- 实现内部可以用本地 `user_id` 关联数据，但 P4 线上对象只使用当前 DID 与可选稳定 Handle；
- Handle-backed Member 应按 `member_handle` 保持成员连续性，并把 `agent_did` 作为可换绑的当前凭证；
- DID-only 成员的 DID 文本变化不具备自动连续性，不得猜测合并；
