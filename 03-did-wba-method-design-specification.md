...
- **service**: Optional field that defines the list of services associated with the DID subject.
  ...
  - **type**: Service type. Currently the following types are supported:
    ...
    - `ANPHandleService`: Handle binding service, used for WNS (WBA Name Space) bidirectional binding verification. `serviceEndpoint` points to the HTTPS endpoint under the Handle Provider domain declared using this DID principal (such as `https://example.com/.well-known/handle/alice`). In v1 of this specification, when the verifier performs reverse binding verification, it only uses the domain part of `serviceEndpoint` to compare it with the domain of the input Handle, and the path does not need to be completely identical; later versions may introduce stronger provider identification mechanisms such as `providerDid`. By declaring `ANPHandleService` in the DID Document, the DID holder confirms the Name Service domain to which its associated Handle belongs, and the verifier can complete bidirectional verification accordingly. For details, see the [ANP DID:WBA Namespace Specification](04-anp-did-wba-name-space-specification.md).
